...
class MockAppProvider extends AppProvider {
  ...
  async initialize(loadConfigFromCache = true, showProjectPicker = true) {
    ...
    if (this.props.ipaConfig.project_name) {
      ...
      let projects = await IafProj.getProjects(
      ...
      if (
        this.props?.sampleSelectedItems?.selectedProject &&
        this.props?.sampleSelectedItems?.selectedUserGroupId
      ) {
        this.setSelectedItems({
    ...
    if (this.props.ipaConfig.user_config_name) {
      const userConfig = currUserGroup._userAttributes.userConfigs.find(
        uc => uc._name == this.props.ipaConfig.user_config_name
      );
      const configs = await IafProj.getUserConfigs(currProject, { _id: userConfig._id });
      config = JSON.parse(
        configs[0]._versions.find(uc => uc._version == configs[0]._tipVersion)._userData
      );
    }
...
export const decorateWithMockAppProvider = (
  Component,
  ipaConfig = {},
  userConfig = {},
  currentPath = '/assets',
  ...
  const LegacyContextProvider = createLegacyContextSupport({

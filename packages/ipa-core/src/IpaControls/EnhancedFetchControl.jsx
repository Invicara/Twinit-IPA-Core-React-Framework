...
class EnhancedFetchControl extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    const selectorsChanged = !_.isEqual(
      _.sortBy(prevProps.selectors),
      _.sortBy(this.props.selectors)
    );
    ...
    const shouldClearSelectorsValues = !_.isEqual(
      prevProps.shouldClearToken,
      this.props.shouldClearToken
    );
  ...
  resetSelectorsMap = (selectors, initialValue) =>
    selectors.reduce(
      (accum, selector, i) => ({
        ...accum,
        [i]: {
          ...
          id: i,
          currentValue:
            initialValue && (initialValue.id == i || initialValue.id == selector.id)
              ? initialValue.value
              : null,
          touched: false,
        },
  ...
  handleFetch = (selectorId, value, state) => {
    ...
    if (this.props.setViewerSelectedEntitiesBySearch)
  ...
  renderControl = (selector, position) => {
    ...
    return (
      <div key={selector.id} onKeyUp={this.keyup}>
  ...
  render() {
    return (
      <div className="enhanced-fetch-control">{this.getSelectors().map(this.renderControl)}</div>
    );
  }
}

...
const treeControlLeafNodeRenderer = group => {
  return (
    <div>
      {parseNodeNameWithParents(group.name).childNodeInfo.displayName}
      {!!group.count && (
...
export const TreeSearch = ({
  ...
  useEffect(() => {
    ...
    const initialRefresh =
      reloadTokenLatest.current == reloadToken && treeLevelsLatest.current == treeLevels;
    const preLoadedTree =
      currentState && !_.isEmpty(currentState) ? { ...currentState } : { ...treeState.nodeIndex };
  ...
  async function getChildrenNames(nodeLevel, node, nodeParentNames) {
    return (await fetchChildren(nodeLevel, node, nodeParentNames)).map(child => child.name);
  }

  const buildNodeIndex = async (
    levelNodes,
    level,
    parents = [],
    parentNames = [],
    ...
    let newNodeIndexPromises = levelNodes.map(async node => {
      ...
      const defaultSelectedStatus =
        _.get(treeState.nodeIndex, `${_.last(parents)}.selectedStatus`) === TreeNodeStatus.ON
          ? TreeNodeStatus.ON
          : TreeNodeStatus.OFF;
  ...
  async function getLevelNodes(level, parentNames = []) {
    ...
    const levelNodes = rawNodesCopy.map((node, position) => {
      ...
      const nameKey = stringifyNodeWithParent(parentBaseName, {
  ...
  async function getChildren(nodeKey, nodeIndex, currentValue) {
    ...
    const levelNodes = await getLevelNodes(level, parentNames);
    let nodeIndicesToAdd = await buildNodeIndex(
      levelNodes,
      level,
      parents,
      parentNames,
  ...
  const propagateSelectStatusDown = (nodeIndex, nodeKey, selected) => {
    ...
    let selectedNodeChildren = selectedNode.children.map(childName =>
      propagateSelectStatusDown(nodeIndex, childName, selected)
    );
  ...
  return (
    <div className="tree-search">
      ...
      {!treeState.reloading && !_.isEmpty(treeState.nodeIndex) ? (
        <ReactiveTreeControl

...
export const parseNodeNameWithParent = (name) => {
    if (!name) throw new InvalidNodeName(name)

    const arr = name.split(NODE_SEPARATOR);

    if (arr.length === 0) throw new InvalidNodeName(name);
    ...
    let mulitLevels = {
        parentNodeInfo: arr.slice(0, -1).map(segment => parseNodeName(segment)),
        childNodeInfo: parseNodeName(arr[arr.length - 1])
    };

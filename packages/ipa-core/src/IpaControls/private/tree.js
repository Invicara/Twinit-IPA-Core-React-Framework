...
export class NotANodeInfo extends Error {
  constructor(property, errorType) {
    super(
      `The value provided is not a NodeInfo object: the property "${property}" should be ${errorType}`
    );
  }
}
...
export const isNodeInfo = obj => {
  ...
  if (obj.displayName.includes(NODE_SEPARATOR))
    throw new NotANodeInfo(
      `displayName (${obj.displayName})`,
      `a string free of '${NODE_SEPARATOR}'`
    );

  if (!_.isInteger(obj.level) || obj.level < 0)
    throw new NotANodeInfo('level', 'a positive integer');

  if (!_.isInteger(obj.position) || obj.position < 0)
    throw new NotANodeInfo('position', 'a positive integer');
};
...
export class InvalidNodeName extends Error {
  constructor(string) {
    super(
      `It looks like there is a '${NODE_SEPARATOR}' in the following node's name: ${string}. It's a reserved character and cannot be used in the node's display name`
    );
  }
}

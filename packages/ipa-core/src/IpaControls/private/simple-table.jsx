...
export const SimpleTableFactory = {
  create: ({ config, data }) => {
    ...
    else return <SimpleTable className={config.className} rows={data} />;
  },
};
...
export const SimpleTabbedTableFactory = {
  create: ({ config, data }) => {
    ...
    if (config.groups) {
      Object.values(config.groups).forEach(props => flattenedGroupedAndHiddenProps.push(...props));
    }
...
export const SimpleTableGroupFactory = {
  create: ({ config, data }) => {
    ...
    const _getRow = obj => {
      ...
      else return ['Configured property does not exist', ''];
    };
    ...
    Object.keys(config.groups).forEach(g => {
      let rows = [];
      config.groups[g]
        .filter(attr => !(config.hidden || []).includes(attr))
        .forEach(attr => {
    ...
    return <div className={'simple-grouped-table ' + config.className}>{components}</div>;
  },
};

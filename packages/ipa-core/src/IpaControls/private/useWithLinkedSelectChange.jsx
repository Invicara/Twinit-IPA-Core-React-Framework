...
export const useWithLinkedSelectChange = (
  selects,
  setSelects,
  currentValue,
  getSelectedValue,
  ...
  const getSubsequentSelects = select => {
    return _.values(selects)
      .slice(select.index + 1)
      .reduce(
        (acc, select) => ({
          ...acc,
  ...
  const handleChange = (selectId, selectedOption, select) => {
    const selectedValues = selectedOption
      ? (select.multi ? selectedOption : [selectedOption]).map(opt => getSelectedValue(opt))
      : [''];

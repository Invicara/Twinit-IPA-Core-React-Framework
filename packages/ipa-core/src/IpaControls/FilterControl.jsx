...
const FUNCTIONS = [
  ...
  // these funcs take text input - all comparisons are case insensitive
  {
    ...
    input: 'text',
    types: '|text|',
    test: (propVal, filterVal) =>
      propVal.toLowerCase().startsWith(new String(filterVal).toLowerCase()),
  },
  {
    name: 'ends with',
    input: 'text',
    types: '|text|',
    test: (propVal, filterVal) =>
      propVal.toLowerCase().endsWith(new String(filterVal).toLowerCase()),
  },
  {
    name: 'contains',
    input: 'text',
    types: '|text|',
    test: (propVal, filterVal) =>
      propVal.toLowerCase().indexOf(new String(filterVal).toLowerCase()) >= 0,
  },
  {
...
const getFormattedDateFromTimestamp = (ts, type) => {
  let d = new Date(ts);
  if (type == 'dateTime') return d.toLocaleString();
...
class FilterControl extends React.Component {
  ...
  render() {
    return (
      <div
        className={
          this.props.className == 'assets-filter' ? 'assets-filter-container' : 'filter-container'
        }
      >
        <CurrentFilters
...
class FilterDropDownPanel extends React.Component {
  ...
  hide = el => {
    while (!el.classList.contains(this.props.parentKey)) el = el.parentElement;
  ...
  add = e => {
    ...
    let type = (filter.op = this.state.selectedFunction);
  ...
  rangeChanged = (input, field) => {
    ...
    if (_.isObject(selectedValue)) selectedValue[field] = num;
    else {
      selectedValue = {};
  ...
  dateRangeChanged = (input, field) => {
    let selectedValue = _.cloneDeep(this.state.selectedValue);
    if (_.isObject(selectedValue)) selectedValue[field] = input ? input.getTime() : null;
    else {
      selectedValue = {};
  ...
  getAvailableFunctions = selectedProperty => {
    ...
    return FUNCTIONS.filter(f => !this.props.functions || this.props.functions.includes(f.name))
  ...
  getAvailableValues = selectedFunction => {
    ...
    if (Array.isArray(this.props.available[this.state.selectedProperty])) {
      return this.props.available[this.state.selectedProperty].map(v => {
        return { value: v, label: v };
      });
    } else if (hasValues) {
      return this.props.available[this.state.selectedProperty].values.map(v => {
        return { value: v, label: v };
      });
    }
  };
  ...
  getDateControl = (type, range) => {
    ...
    if (range == false)
      ...
      value = {};
      value.from =
        this.state.selectedValue && this.state.selectedValue.from
          ? new Date(this.state.selectedValue.from)
          : null;
      value.to =
        this.state.selectedValue && this.state.selectedValue.to
          ? new Date(this.state.selectedValue.to)
          : null;
    }

    if (type == 'date') {
      if (range == false) {
        return <DatePicker onChange={this.dateChanged} value={value} calendarIcon={null} />;
      } else {
        return (
      ...
      if (range == false) {
        return <DateTimePicker onChange={this.dateChanged} value={value} calendarIcon={null} />;
      } else {
        return (
          <span className="range">
            <label>From</label>
          ...
          <Select
            ...
            value={
              this.state.selectedValue
                ? { value: this.state.selectedValue, label: this.state.selectedValue }
                : null
            }
      ...
      case 'multi-pick':
        return hasValues ? (
          <Select
            ...
            value={
              this.state.selectedValue
                ? this.state.selectedValue.map(v => {
                    return { value: v, label: v };
                  })
                : null
            }
          ...
          return (
            <span className="range">
              ...
              <input
                ...
                value={
                  this.state.selectedValue && this.state.selectedValue.from !== undefined
                    ? this.state.selectedValue.from
                    : ''
                }
                placeholder="from"
                className="filter-input-value"
  ...
  render() {
    let properties = Object.keys(this.props.available)
      .sort()
      .map(a => {
        return { label: a + (this.props.filters[a] ? ' *' : ''), value: a };
      });
    ...
    return (
      <div className={'hidden filter-drop-down-panel ' + this.props.parentKey}>
          ...
          <Select
            ...
            value={
              this.state.selectedProperty
                ? { value: this.state.selectedProperty, label: this.state.selectedProperty }
                : null
            }
          ...
          <Select
            ...
            value={
              this.state.selectedFunction
                ? { value: this.state.selectedFunction, label: this.state.selectedFunction }
                : null
            }
        ...
        <div>{this.getInputControl()}</div>
        <GenericMatButton onClick={this.cancel}>Cancel</GenericMatButton>
        <GenericMatButton disabled={!this.state.isReady} onClick={this.add}>
          Add
        </GenericMatButton>
      </div>
    );
      ...
      result = result.filter(a => {
        ...
        if (!propVal) {
          if (filter.value == '_empty_' && (filter.op == 'equals' || filter.op == 'in'))
            return true;
          if (
            filter.value != '_empty_' &&
            (filter.op.startsWith('does not') || filter.op == 'is not in')
          )
            return true;
          else return false;
        }

...
const addTitle = (title) => <h1 key={getRandomString("ice-title-")}style={{position: "absolute", width: "100%", textAlign:"center"}}>{title}</h1>
...
const addCenterLabel = (config, data) => {
  ...
  return (
    <div key={getRandomString("ice-center-label-")} style={{position: "absolute", width: "100%", height: "100%"}}>
      <div
        style={{
          position: "absolute",
...
const getChartExtensions = (chartConfig, data) => {
  let exConfigKeys = Object.keys(chartConfig).filter(k => k.startsWith("ice"))

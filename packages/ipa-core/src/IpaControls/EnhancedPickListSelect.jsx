...
export const EnhancedPickListSelect = ({
  ...
  const fetchOptions = async (currentSelect, currentSelectValue) => {
    ...
    if (nextSelect) {
      const selectOptions = await ScriptCache.runScript(pickListScript, {
        type: currentSelect
          ? currentSelectValue[currentSelect.display][0].value
          : initialPickListType,
      });
  ...
  const updatePickList = async (select, displayValue, optionValue) => {
    let scriptArgs = {
      type: select.type,
      newValue: { display: displayValue, value: optionValue },
    };
  ...
  const handleCreatableChange = (selectId, selected, select) => {
    if (selected && selected.__isNew__) {
      ...
      const newSelectValue = select.createPickListOnUpdate
        ? selected.value + select.type
        : selected.value;
          ...
          <Fragment key={selectId}>
            ...
            {canCreateItems && (
              <CreatableSelect
                ...
                closeMenuOnSelect={true}
                isClearable={isClearable}
                placeholder={`Create or Select a ${selectId}`}
                menuPlacement="auto"
                menuPosition="fixed"

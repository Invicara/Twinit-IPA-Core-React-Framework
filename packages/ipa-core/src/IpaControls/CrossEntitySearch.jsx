...
const CrossEntitySearch = ({ searchableEntities = [], script, dashboard: { doAction } }) => {
  ...
  const onFetch = useCallback(async () => {
    setFetching(true);
    setFetchedEntities(
      await ScriptHelper.executeScript(script, {
  ...
  const searchEntities = (
    <div>
      ...
      <div className={'entity-types'}>
        {rawSearchableEntities.map(et => (
          <div key={et.entityType.plural}>
            <RoundCheckbox checked={et.checked} onChange={() => handleCheck(et)} />
  ...
  const navigate = useCallback(
    entity => {
      const currentSearchableEntity = rawSearchableEntities.find(
        e => e.entityType.plural === entity.entityType
      );
      doAction({
  ...
  return (
    <div className={'cross-entity-search'}>
      ...
      <div className={clsx('search-results', _.isEmpty(fetchedEntities) && 'collapsed')}>
        <EntityListView config={config} entities={fetchedEntities} onDetail={navigate} />
      </div>
    </div>
  );
};

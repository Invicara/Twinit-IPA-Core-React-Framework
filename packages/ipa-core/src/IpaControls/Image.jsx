...
const Image = ({ script, url, filename, styles, navigateTo, query, dashboard, padding }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        else if (filename) resolvedUrl = await FileHelpers.getFileUrlForFilename(filename);
        else if (script) {
  ...
  if (loading) {
    ...
    component = (
      <div className="configured-image-placeholder" role="img" aria-label="No image available">
        <svg
          viewBox="0 0 24 24"
          width="48"
          height="48"
  ...
  return (
    <div style={{ padding: padding ? padding : '15px', width: '100%', height: '100%' }}>
      <div className="configured-image-component">{component}</div>
    </div>
  );
};

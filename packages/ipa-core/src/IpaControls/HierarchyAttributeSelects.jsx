...
export default class HierarchyAttributeSelects extends React.Component {
  ...
  async handleChange(selectedValues) {
    ...
    //recursively populate the options in the selects from the structured data
  ...
  async buildSelects() {
    ...
    for (let i = 0; i < this.props.hierarchyAttributes.length; i++) {
      selects.push({
        ...
        isMulti:
          this.props.options &&
          this.props.options.isMulti &&
          i === this.props.hierarchyAttributes.length - 1,
  ...
  async componentDidUpdate(prevProps, prevState) {
    if (
      ...
      !!prevProps.defaultSelections &&
      !_.isEqual(prevProps.defaultSelections, this.props.defaultSelections)
    ) {
      await this.buildSelects();
    }
  }

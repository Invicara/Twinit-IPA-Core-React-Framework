...
export const StackableDrawer = ({
  ...
  const toggleOpen = useCallback(() => {
    ...
    setStableWidth(stableWidth =>
      stableWidth === 0 ? (fixedWidth != 0 ? fixedWidth : DEFAULT_DRAWER_WIDTH) : 0
    );
  }, [stableWidth]);
  ...
  return (
    <div ref={drawer} className={'drawer'} style={{ width: stableWidth, minWidth: stableWidth }}>
      {iconKey && (
        <div
          style={{ top: `${20 + toggleHeight * (level - 1)}px` }}
          className={clsx({ 'drawer-toggle': true, 'drawer-toggle-open': open })}
          onClick={toggleOpen}
        >
          {tooltip ? (

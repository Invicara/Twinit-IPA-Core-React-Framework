...
export default class LinkedSelectsProgressive extends React.Component {
  ...
  async componentDidMount() {
    ...
    if (
      !!this.props.options &&
      !!this.props.options.direction &&
      this.props.options.direction === 'vertical'
    )
  ...
  componentDidUpdate(prevProps) {
    if (
      ...
      !_.isEqual(this.props.defaultSelections, prevProps.defaultSelections)
    )
      this.updateValueOptions();
  }
  ...
  async handleSelectValue(att, options, index) {
    ...
    if (att.isMulti) {
      ...
      if (!options)
        valueOptions[att.name] = []; //when a single select if cleared options is null
      else valueOptions[att.name] = [options];
    }
  ...
  render() {
    ...
    let selectsControls = this.props.attributes
      ? this.props.attributes.map((att, index) => {
          let selectDisabled = false;
          if (att.values.length === 0) selectDisabled = true;
          else if (
            index !== 0 &&
            (!this.state.valueOptions[this.props.attributes[index - 1].name] ||
              this.state.valueOptions[this.props.attributes[index - 1].name].length === 0)
          )
            selectDisabled = true;
          else if (this.props.disabled) selectDisabled = true;

          return (
            <div key={att.name} style={subcontainerStyles}>
              {!!this.props.options && this.props.options.hasLabel && (
                <label
                  style={{ display: 'block', margin: '10px', fontWeight: 'bold' }}

...
const wrapWithTooltip = (type, element) => {
  ...
  return (
    <Tooltip key="cell-tooltip" title={element} placement="bottom-start" enterDelay={500}>
      {element}
    </Tooltip>
  );
};
...
export const getCellContent = (type, val) => {
  switch (type) {
    ...
    case 'tags':
      return (
        <div className="cell__content cell__content--tags">
          {val.map((tag, idx) => {
            return (
              <span key={idx} className="cell__content cell__content--tag">
                {tag}
              </span>
            );
          })}

...
const FancyTreeControl = ({
  ...
  const [selectAllState, dispatch] = useReducer(
    fancyTreeControlSelectAllReducer,
  ...
  useEffect(() => {
    ...
    if (allSelected) {
      if (treeSelectMode === TreeSelectMode.NONE_MEANS_ALL) {
        ...
        Array.from(selectedNodes).forEach(s => s.classList.remove('selected'));
        ...
        Array.from(deselectedNodes).forEach(s => s.classList.add('selected'));
      }
    }
  ...
  useEffect(() => {
    ...
    if (_.isEmpty(selectedIds) && _.isEmpty(selectedNodeNames)) {
      ...
      Array.from(allSelected).forEach(s => s.classList.remove('selected'));
    }
  }, [selectedIds]);
  ...
  const getNodes = (nodes, depth) => {
    ...
    if (Array.isArray(nodes)) {
      children = nodes.map(n => {
        ...
        return (
          <li
            onClick={e => selectNode(e, n.name, n)}
            key={n._id || n.name}
            data-node-id={n._id}
            className={cn}
          >
      ...
      Object.entries(nodes).forEach(([nodeName, nodeValue]) => {
        children.push(
          <li
            className={clsx(
              'branch',
              selectedNodeNames.includes(nodeName) && 'selected',
              expandedNodeNames.includes(nodeName) && 'expanded',
              partialNodeNames.includes(nodeName) && 'partial'
            )}
            onClick={e => selectNode(e, nodeName, nodeValue)}
            key={nodeName}
          ...
          >
            <a>
              <span>
                <i
                  ...
                  onClick={e => expandBranch(e, nodeName, nodeValue)}
                />
            ...
            <ul
              style={{
                height: nodeValue.length > 50 ? '60vh' : 'auto',
                overflow: nodeValue.length > 50 ? 'auto' : 'none',
              }}
              key={nodeName + '_children'}
            >
              {getNodes(nodeValue, depth)}
            </ul>
          </li>
        );
      });
  ...
  return (
    <div className={'fancy-tree'} ref={treeDOMRef}>
      {onSelectAll ? (
        <div className="fancy-tree__count">
          <span>{`Refined: ${selectedIds.length}`}</span>
          {!allSelected ? (
            <span className="fancy-tree__count--action" onClick={onSelectAllAction}>
              Select All
            </span>
          ) : onSelectNoneAction ? (
            <span className="fancy-tree__count--action" onClick={onSelectNoneAction}>
              Deselect All
            </span>
          ) : undefined}
          {selectAllState.selectAllActivated && selectAllState.previouslySelectedIds.length > 0 ? (
            <span className="fancy-tree__count--action" onClick={onUndoSelectAllAction}>
              Undo
            </span>
          ) : undefined}
        </div>

...
const Select = props => {
  ...
  return (
    <div className="ipa-select">
      {props.labelProps && <ControlLabel {...props.labelProps} required={props.required} />}
      <SelectComponent {...selectProps} />
    </div>
  );
};

export const highlightOptions = (keysToHighlight, options) =>
  options.map(option => ({
    ...option,
...
Select.propTypes = {
  ...
  onChange: func,
  options: array,
  closeMenuOnSelect: oneOfType([func, bool]),
  isClearable: bool,
  placeholder: string,
  label: string,
  isDisabled: bool,

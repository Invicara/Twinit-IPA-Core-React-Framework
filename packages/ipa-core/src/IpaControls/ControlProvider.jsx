...
const getScriptedSelectQuery = (selector, value) => {
  ...
  if (hasMultiProps) {
    query[op] = props.map(prop => {
      ...
      let propName =
        selectConfig && selectConfig.propName
          ? selectConfig.propName
          : 'properties.' + prop + '.val';
      let qOption = {};
...
const getTreeSelectQuery = (selector, filteringNodes) => {
  const getQueryNodesFor = nodeNames => {
    return {
      $or: _.values(_.pick(filteringNodes, nodeNames)).map(node =>
        !node.isLeaf
          ? {
              $and: [
                {
                  [`properties.${selector.treeLevels[node.level].property}.val`]:
                    parseNodeNameWithParent(node.name).childNodeInfo.displayName,
                },
...
const buildLeafQuery = (selector, node) => {
  ...
  if (node.parents?.length > 0 && selector.treeLevels[parentLevel]) {
    node.parents.forEach((p, idx) => {
      query.push({
        [`properties.${selector.treeLevels[idx].property}.val`]:
          parseNodeNameWithParent(p).childNodeInfo.displayName,
      });
    });
  }

...
const ReactiveTreeControl = ({
  nodeIndex,
  onNodeIndexChange,
  renderBranchNode = defaultBranchRenderer,
  renderLeafNode = defaultLeafRenderer,
  nodeHeight = 30,
}) => {
  ...
  const expandBranch = node =>
    onNodeIndexChange(
      produce(nodeIndex, nodeIndex => {
        nodeIndex[node.id].expanded = !nodeIndex[node.id].expanded;
      })
    );

  const toggleNode = property => node =>
    onNodeIndexChange(
      produce(nodeIndex, nodeIndex => {
        const wasOn = nodeIndex[node.id][property] === TreeNodeStatus.ON;
        propagateNodeStatusDown(property)(
          nodeIndex,
          node.id,
          wasOn ? TreeNodeStatus.OFF : TreeNodeStatus.ON
        );
  ...
  const getChildrenCount = node =>
    _.values(nodeIndex).filter(n => n.isLeaf && n.parents.includes(node.id)).length;
  ...
  const renderBranch = node => {
    ...
    return (
      <li
        ...
        onClick={withoutPropagation(() => toggleNode('selectedStatus')(node))}
        key={node.id}
      >
        <a>
          <span>
            <i
              ...
              onClick={withoutPropagation(() => expandBranch(node))}
            />
        ...
        {isLeaf ? (
          <ul
            ...
            style={{ maxHeight: `${MAX_HEIGHT_SCROLL}vh`, overflowY: 'auto' }}
          >
  ...
  return (
    <div className={'fancy-tree'}>
      {!_.isEmpty(nodeIndex) ? (
        <ul>{renderNodes(_.values(nodeIndex).filter(node => node.level === 0))}</ul>
      ) : null}
    </div>
  );
};

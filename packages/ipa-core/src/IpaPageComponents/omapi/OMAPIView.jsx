...
const OMAPIView = ({ onLoadComplete, selectedItems, token }) => {
  ...
  useEffect(() => {
    async function fetchApiConfig() {
      ...
      if (!configs || !configs.length) {
        ...
        try {
          ...
          if (!apiConfig) {
            setError(
              'Object Model API Configuration Error: Error parsing Object Model API config!'
            );
          } else {
    ...
    setRootUrl(
      endPointConfig.itemServiceOrigin + '/omapi/' + selectedItems.selectedProject._namespaces[0]
    );
    fetchApiConfig();
  }, []);
  ...
  return (
    <div>
      {ready && (
        <div className="omapi-page">
          <div className="page-header">
            <h1>Object Model API Documentation</h1>
          </div>
          {error && (
          ...
          <div>
            {apiConfig &&
              apiConfig.endpoints &&
              Object.keys(apiConfig.endpoints).map(e => {
                return (
                  <div key={e}>
                    <hr />
                    <div className="path-header" onClick={() => toggleExpand(e)}>
                      ...
                      <h2>{e}</h2>
                      {!hasOptionsMethod(e) && (
                        <span className="missing-options">
                          <i className="fas fa-exclamation-circle fa-2x"></i> Resource is missing
                          OPTIONS path!
                        </span>
                      )}
                    </div>
                    {expanded[e] && (
                      <ul>
                        {apiConfig.endpoints[e].paths
                          .filter(p => p.method !== 'OPTIONS')
                          .sort()
                          .map(p => (
                            <OMAPICard
                              key={p.path + '-' + p.method}

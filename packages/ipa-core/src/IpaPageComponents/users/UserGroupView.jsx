...
class UserGroupView extends React.Component {
    ...
    return new Promise((resolve, reject) => [
      IafProj.getUserGroups(this.props.selectedItems.selectedProject).then(async groups => {
        ...
        IafProj.getUserConfigs(this.props.selectedItems.selectedProject, {
          query: { _userType: this.props.selectedItems.ipaConfig.configUserType },
        }).then(userConfigs => {
  ...
  async canRemoveUser(user = false, group = false) {
    ...
    if (user) {
      if (
        this.state.selectedUserGroup._id === this.props.selectedItems.selectedUserGroupId &&
  ...
  async removeUser(user = false, group = false) {
    ...
    let canRemove;
    if (this.state.pageMode === 'UserGroups') canRemove = await this.canRemoveUser(removeUser);
  ...
  render() {
    return (
      <div className="user-group-view">
        {this.props.handler.config.allowUserGroupInvite && (
          <StackableDrawer level={1} iconKey="fas fa-user-plus" isDrawerOpen={false}>
            <InviteForm
        ...
        >
          <div className="switchable-list-view">
            ...
            <hr />
            {this.state.pageMode === 'UserGroups' && (
              <div>
        ...
        {this.state.pageMode === 'UserGroups' && (
          <div className="usergroup-mode-view">
              ...
              {!this.state.editingUserGroup && (
                <div className="usergroup-info">
                  <div className="usergroup-name">
                    <h1>
                      {this.state.selectedUserGroup ? this.state.selectedUserGroup._name : ''}
                    </h1>
                    {this.props.handler.config.allowUserGroupEdit &&
                      this.state.selectedUserGroup && (
                  ...
                  {this.props.handler.config.allowViewPermissions && (
                    <div>
                      <RadioButtons
              ...
              {this.state.editingUserGroup && (
                <div>
                  <div className="usergroup-name editable">
                    ...
                    <span className="ug-btn">
                      {!this.state.savingUserGroup && !this.state.userGroupNameEditError && (
                  ...
                  {this.state.userGroupNameEditError && (
                    <div className="usergroup-name-error">{this.state.userGroupNameEditError}</div>
                  )}
                </div>
              )}
            </div>
            ...
            {this.state.userGroupMode === 'Users/Invites' && (
              <div className="row2">
                ...
                <div className="usergroup-invites">
                  ...
                  {this.state.loadingInvites && (
                    <div className="throbber">
                      <SimpleTextThrobber throbberText="Loading UserGroup Invites" />
                    </div>
                  )}
                  {!this.state.loadingInvites && this.state.invitesInSelectedGroup.length === 0 && (
                    <span className="indent-header">No pending invites</span>
                  )}
                  <ul>
                    {this.state.invitesInSelectedGroup.map(i => (
                      <InviteCard
                        key={i._id}
                        invite={i}
                  ...
                  {this.state.expiredInvitesInSelectedGroup.length > 0 && (
                    <div>
                      <span className="indent-header">Expired Invites</span>
                      <ul>
                        {this.state.expiredInvitesInSelectedGroup.map(i => (
                          <InviteCard
                            key={i._id}
                            invite={i}
                            isCurrentUser={i._email === this.props.user._email}
            ...
            {this.state.userGroupMode === 'Permissions' && (
          ...
          <div className="user-mode-view">
              ...
              <div className="user-name-title">
                <h1>
                  {this.state.selectedUser
                    ? this.state.selectedUser._lastname + ', ' + this.state.selectedUser._firstname
                    : ''}
                </h1>
              </div>
            </div>
            ...
            <div className="row2">
              <div className="member-usergroups">
                ...
                {this.state.userGroupsForSelectedUser.length === 0 && (
                  <div className="throbber">
                    <SimpleTextThrobber throbberText="Loading UserGroups" />
                  </div>
                )}
                <ul className="member-usergroup-list">
                  {this.state.userGroupsForSelectedUser.map(u => (
                    <GroupCard
                      key={u._id}
                      group={u}
                      showActions={this.props.handler.config.allowManageUsers}
              ...
              <div className="member-invites">
                ...
                {!this.state.loadingInvites && this.state.invitesForSelectedUser.length === 0 && (
                  <span className="indent-header">No pending invites</span>
                )}
                <ul>
                  {this.state.invitesForSelectedUser.map(i => (
                    <InviteCard
                      key={i._id}
                      invite={i}
                      isCurrentUser={i._email === this.props.user._email}
                ...
                {this.state.expiredInvitesForSelectedUser.length > 0 && (
                  <div>
                    <span className="indent-header">Expired Invites</span>
                    <ul>
                      {this.state.expiredInvitesForSelectedUser.map(i => (
                        <InviteCard
                          key={i._id}
                          invite={i}
                          isCurrentUser={i._email === this.props.user._email}

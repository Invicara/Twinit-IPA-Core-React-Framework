...
export const handleOnLoad = (setActions, itemFetchScript, setAllUserItems) => {
  useEffect(() => {
...
export const hasAllAccess = (where, accessAll, setAccessAll) => {
  let hasAccessAll =
    accessAll ||
    !!_.find(where, { _resourceDesc: { _irn: IafPermission.AllAccessIrn } });
  if (hasAccessAll !== accessAll) setAccessAll(hasAccessAll);
  return hasAccessAll;
};

export const createRowForTable = (item, permissions, all) => {
  let allAction = false;
...
export const getFilesWithPermissions = (
  accessAll,
  hasAllAccess,
  filePerms,
  setAccessAll,
  ...
  let hasAllPermissions = hasAllAccess(filePerms, accessAll, setAccessAll);
  ...
  let permissionsObj = _.find(filePerms, {
...
export const getUserItemsWithPermissions = (
  allUserItems,
  usergroupItemPerms,
  hasAllAccess,
  createRowForTable,
  ...
  function getPermsForItem(item, usergroupItemPerms) {
    let topLevelNamedUserItemPerms = usergroupItemPerms.filter(
      (p) =>
        p._resourceDesc._irn === "itemsvc:nameduseritem:*" &&
        !p._resourceDesc._criteria,
    );

    let itemClassItemPerms = usergroupItemPerms.filter(
      (p) =>
        p._resourceDesc._criteria &&
        p._resourceDesc._criteria._itemClass === item._itemClass,
    );

    let userTypeItemPerms = usergroupItemPerms.filter(
      (p) =>
        p._resourceDesc._criteria &&
        p._resourceDesc._criteria._userType === item._userType,
    );

    let topLevelActions = _.flatten(
      topLevelNamedUserItemPerms.map((p) => p._actions),
    );
  ...
  if (Array.isArray(allUserItems)) {
    tableRows = allUserItems.map((item) => {
      if (hasAllPermissions) return createRowForTable(item, null, true);
      else {
        let itemPermissions = getPermsForItem(item, usergroupItemPerms);
...
export const handleUserGroup = (
  ...
  useEffect(() => {
    async function getUserGroupPermissions() {
      let query = {
        _namespace: usergroup
          ? usergroup._userAttributes.project_workspace._namespaces
          : "",
        "_user._id": usergroup ? usergroup._id : "",
...
export const getItemFilterOptions = (allUserItems, setItemFilterOptions) => {
  let newFilterValues = [];
  if (Array.isArray(allUserItems)) {
    newFilterValues = _.uniq(allUserItems.map((u) => u._itemClass));
  }
...
export const handleUserItems = (
  getItemFilterOptions,
  allUserItems,
  setItemFilterOptions,
) => {
  useEffect(() => {
    getItemFilterOptions(allUserItems, setItemFilterOptions);
  }, [allUserItems]);
};

export const handleFilesPerms = (
  filePerms,
  accessAll,
  hasAllAccess,
  setAccessAll,
...
export const onFilterChange = (
  ...
  if (f.value === "All") setItemFilteredTableData(itemTableData);
  else {
    let filteredRows = itemTableData.filter(
      (r) => r.item._itemClass === f.value,
    );
...
export const UserGroupPermissionTable = ({
  usergroup,
  ...
  handleOnLoad(setActions, itemFetchScript, setAllUserItems);
  handleUserGroup(
    usergroup,
    setUserGroupItemPerms,
    setFilePerms,
  ...
  handleUserItems(getItemFilterOptions, allUserItems, setItemFilterOptions);
  handleItemPerms(
    usergroupItemPerms,
    allUserItems,
    accessAll,
  ...
  return (
    <div className="permissions-table">
      ...
      {loadingFilePerms && (
        <SimpleTextThrobber throbberText="Loading File Permissions" />
      )}
      {!loadingFilePerms && (
          ...
          <tbody>
            {fileTableData.map((row, index) => {
              return (
                ...
                >
                  <td>
                    <div className="ug-perm-table-row-itemname">
                      {row.item._name}
                    </div>
                    <div className="ug-perm-table-row-itemdesc">
                      {row.item._description}
                    </div>
                  </td>
                  {actions.map((a) => (
                    <td key={row.name + a} className="perm-cell">
                      {!accessAll && allowManagePermissions && (
                        <RoundCheckbox checked={row[a]} />
                      )}
                      {(accessAll || !allowManagePermissions) && (
                        <i
                          className={clsx(
                            row[a] && "fas fa-check-circle",
                            !row[a] && "far fa-circle",
                          )}
                        ></i>
                      )}
      ...
      {loadingItemPerms && (
        <SimpleTextThrobber throbberText="Loading Item Permissions" />
      )}
      {!loadingItemPerms && (
        <table className="ug-perm-table" data-testid="table">
          <thead>
            <tr className="ug-perm-table-header">
              <th>Name</th>
              <th>Class</th>

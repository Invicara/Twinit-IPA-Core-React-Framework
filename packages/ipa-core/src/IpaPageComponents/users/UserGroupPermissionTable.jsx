...
export const UserGroupPermissionTable = ({
  ...
  const hasAllAccess = where => {
    let hasAccessAll =
      accessAll || !!_.find(where, { _resourceDesc: { _irn: IafPermission.AllAccessIrn } });
  ...
  const getUserItemsWithPermissions = () => {
    function getPermsForItem(item) {
      let topLevelNamedUserItemPerms = usergroupItemPerms.filter(
        p => p._resourceDesc._irn === 'itemsvc:nameduseritem:*' && !p._resourceDesc._criteria
      );

      let itemClassItemPerms = usergroupItemPerms.filter(
        p => p._resourceDesc._criteria && p._resourceDesc._criteria._itemClass === item._itemClass
      );

      let userTypeItemPerms = usergroupItemPerms.filter(
        p => p._resourceDesc._criteria && p._resourceDesc._criteria._userType === item._userType
      );
      ...
      let combinedPerms = [...topLevelActions, ...itemClassActions, ...userTypeActions];
        ...
        <table className="ug-perm-table">
          ...
          <tbody>
            {fileTableData.map((row, index) => {
              return (
                <tr key={index} className={clsx('ug-perm-table-row', index % 2 !== 0 && 'odd-row')}>
                  <td>
      ...
      <div className="ug-perm-table-ctrls">
        ...
        <div className="ug-perm-filter">
          ...
          <div style={{ minWidth: '300px' }}>
            <Select options={itemFilterOptions} value={itemFilterValue} onChange={onFilterChange} />
          </div>
        </div>
      </div>

  ...
  const EntityStoreHOC = class extends React.Component {
    ...
    initStoreValues = initialEntityType => {
      ...
      if (initialEntityType) {
        const isCurrentEntityType =
          queryParams && queryParams.entityType === initialEntityType.singular;
    ...
    deriveInitialEntityType = queryParams => {
      ...
      if (
        this.props.currentEntityType &&
        _.includes(this.props.allowedEntityTypes, this.props.currentEntityType?.singular)
      ) {
      ...
      if (queryParams.entityType !== queryParams.senderEntityType) {
        ...
        const selectedEntityOption = entityOptions.filter(
          entityConfig => entityConfig.singular === queryParams.entityType
        );
    ...
    updateEntityType = args => {
      ...
      if (currentEntity.singular !== singular || currentEntity.plural !== plural) {
        const newEntityConfig = _.includes(this.props.allowedEntityTypes, singular)
          ? _.values(this.props.perEntityConfig)[singular]
          : {};
    ...
    getWrappedComponent = wrappedProps => (
      <WrappedComponent

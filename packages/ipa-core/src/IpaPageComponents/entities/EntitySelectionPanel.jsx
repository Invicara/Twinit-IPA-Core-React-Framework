...
class EntitySelectionPanel extends React.Component {
  ...
  saveGroups(props) {
    ...
    sessionStorage.setItem(
      LAST_SELECTED_GROUP_KEY + props.entitySingular + projectId,
      groupsToSave
    );
  }
  ...
  componentDidMount() {
    ...
    if (this.props.setFilteredBySearchEntities)
      this.props.setFilteredBySearchEntities(this.props.isolatedEntities);
  }
  ...
  static getDerivedStateFromProps(props, state) {
    ...
    derivedState.availableFilters = getAvailableFilterValues(
      props.entities,
      derivedState.uniquePropNames,
      props.nonFilterableProperties
    );
    ...
    if (this.props.fetching || !this.props.entities || this.props.entities.length == 0) {
      return (
        <div className="entity-tree-panel">
          <div className="centered">{this.props.fetching ? 'Retrieving data' : 'No data'}</div>
        </div>
      );
    }

    const countMessage =
      this.state.numFilteredEntities == this.state.numEntities ? (
        <span>
          {this.state.numEntities}{' '}
          {this.state.numEntities == 1 ? this.props.entitySingular : this.props.entityPlural}
        </span>
      ) : (
        <span>
          <b>{this.state.numFilteredEntities}</b> of {this.state.numEntities} Fetched{' '}
          {this.state.numEntities == 1 ? this.props.entitySingular : this.props.entityPlural}
        </span>
      );
      ...
      <div className="entity-tree-panel">
        ...
        <FancyTreeControl
          ...
          selectedIds={getSelectedIds(
            ...
            this.props.filteredBySearchEntityIds
          )}
...
const getUniquePropNames = entities => {
  ...
  entities.forEach(a =>
    Object.keys(a.properties || {}).reduce((s, e) => s.add(e), uniquePropNames)
  );
...
const getAvailableFilterValues = (entities, uniquePropNames, nonFilterableProps) => {
  ...
  uniquePropNames
    .filter(p => !nonFilterableProperties.includes(p))
    .forEach(p => {
...
const makeTree = (props, state) => {
  ...
  if (groups && groups.length)
    tree = nestedGroup(filteredEntities, groups, (a, p) =>
      a.properties[p] ? a.properties[p].val : null
    );

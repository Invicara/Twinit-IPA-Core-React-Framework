...
class EntityDetailPanel extends React.Component {
  ...
  render() {
    ...
    dataGroups = dataGroups
      .sort()
      .map(n => n.substring(4))
      .filter(n => availableDataGroups && availableDataGroups[n]);
    return (
...
const EntityDataGroupSelector = ({
  ...
  let availableGroups = dataGroups
    ? Object.entries(dataGroups)
        .filter(([k, v]) => v === true)
        .map(([k, v]) => k)
    : [];
  availableGroups = availableGroups.map(k => {
    return { k: k, index: Object.keys(config.data).findIndex(n => n === k) };
  });

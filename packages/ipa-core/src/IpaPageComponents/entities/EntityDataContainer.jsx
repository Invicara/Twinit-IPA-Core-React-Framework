...
export const useEntityData = (collapsable, collapsed, entity, config, getData, dataGroupName) => {
  ...
  const key = useMemo(
    () => JSON.stringify([collapsable, collapsed, entity, config, dataGroupName]),
    [collapsable, collapsed, entity, config, dataGroupName]
  );
  ...
  return {
    reset,
    reload,
    data: data[key]?.data === undefined ? null : data[key]?.data,
    fetching: data[key]?.fetching === undefined ? true : data[key].fetching,
    error: data[key]?.error === undefined ? null : data[key].error,
  };
};

const EntityDataContainer = props => {
  const [collapsed, setCollapsed] = useState(
    props.config.hasOwnProperty('isProperties') ? !props.config.isProperties : true
  );
  ...
  return (
    <div className="entity-data-container">
      ...
      {!props.collapsable || (props.collapsable && !collapsed) ? (
        <div
          className={clsx({

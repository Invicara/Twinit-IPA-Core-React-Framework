  ...
  const EntityConfigHOC = class extends React.Component {
    ...
    _getPerEntityConfig = _.memoize(currentConfig => {
      if (this._allowsMultipleEntityTypes(currentConfig)) {
        ...
        const consolidatedConfig = _.mergeWith(
          { ...entityData },
          { ...selectBy },
          (entityData, selectors, key) => {
        ...
        let result = _.mapValues(consolidatedConfig, (entityConfig, entityName) => ({
          ...entityConfig,
          ...currentConfig.type.find(t => t.singular === entityName),
        }));
        return result;
      } else {
    ...
    getEntityExtendedDataFetcher = extendedDataConfig => {
      ...
      return async (dataType, entityInfo) => {
        ...
        let result = await ScriptCache.runScript(
          scriptName,
    ...
    getExtendedDataConfig = _.memoize(
      currentConfig => () => this._getExtendedDataConfig(currentConfig)
    );
    _getExtendedDataConfig = _.memoize(currentConfig => {
      ...
      if (currentConfig.data) {
        ...
        if (dataTypes.length) {
          ...
          let consolidatedExtendedData = _.values(currentConfig.data).reduce((acc, current) => ({
            ...acc,
            ...current,
          }));
    ...
    findAvailableDataGroups =
      currentConfig => (entity, propertiesOnly, entitySingular, onDataGroup, onFinishLoading) => {
        if (!propertiesOnly) {
    ...
    getDataGroupFetcher = (entityType, entity, dataGroup, dataGroupName, onDataGroupLoaded) => {
      return ScriptCache.runScript(
        ...
        .then(extendedData => {
          let isAvailable;
          if (!extendedData) isAvailable = false;
          else if (Array.isArray(extendedData) && extendedData.length > 0) isAvailable = true;
          else if (Object.keys(extendedData).length > 0) isAvailable = true;

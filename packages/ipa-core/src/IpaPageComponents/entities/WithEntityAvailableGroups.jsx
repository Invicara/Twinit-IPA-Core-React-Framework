...
const withEntityAvailableGroups = WrappedComponent => {
  const EntityGroupsHOC = class extends React.Component {
    ...
    setAvailableDataGroups = (entity, propertiesOnly, detailedEntity) => {
      ...
      if (entity) {
        this.props.findAvailableDataGroups(
          detailedEntity ? detailedEntity : entity,
          propertiesOnly,
          entity ? this.props.currentEntityType.singular : undefined,
          this.onDataGroupAvailable,
          this.onDataGroupsLoaded
        );
      }
    };
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
      ...
      if (
        ...
        prevProps.detailedEntity !== this.props.detailedEntity
      ) {
        this.selectedEntitiesEffect();
      }
    }
    ...
    selectedEntitiesEffect() {
      if (this.props.selectedEntities.length) {
        this.setAvailableDataGroups(
          this.props.selectedEntities[0],
          false,
          this.props.detailedEntity
        );
      }
    }
    ...
    render() {
      ...
      return (
        <WrappedComponent {...wrappedProps} setAvailableDataGroups={this.setAvailableDataGroups} />
      );
    }
  };

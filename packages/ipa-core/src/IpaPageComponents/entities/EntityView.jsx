...
class EntityView extends React.Component {
  ...
  tableEntities = () =>
    _.isEmpty(this.props.selectedEntities)
      ? this.props.currentEntities
  ...
  actionSuccess = (actionType, newEntity, result) => {
    ...
    if (actionType === 'edit') {
      let updatedEntities = this.props.allEntities.map(a => {
      ...
      if (
        ...
        !_.isEmpty(filteredEntities.find(e => e._id === newEntity._id))
      )
        this.openSummary();
    }
  ...
  render() {
    ...
    let query = Object.assign(
      {},
      this.props.queryParams && this.props.queryParams.query ? this.props.queryParams.query : {}
    );
    ...
    const nonGroupableProps =
      (this.props.handler.config.entitySelectionPanel
        ? this.props.handler.config.entitySelectionPanel.nonGroupableProperties
        : []) || [];
    const nonFilterableProps =
      (this.props.handler.config.entitySelectionPanel
        ? this.props.handler.config.entitySelectionPanel.nonFilterableProperties
        : []) || [];
    const defaultGroups =
      (this.props.handler.config.entitySelectionPanel
        ? this.props.handler.config.entitySelectionPanel.defaultGroups
        : []) || [];

...
import { LinkTwoTone as LinkTwoToneIcon, LinkOff } from '@mui/icons-material';
...
export const SummaryPanel = ({
  ...
  const getFilteredRelated = entity =>
    selectedEntityType
      ? entity.related.filter(r => r.entityType === selectedEntityType)
      : entity.related;

  return (
    <div className={'panel relations-modal-summary'}>
      <div className={'panel-title'}>Summary</div>
      {fetching ? (
        <FetchingLegend legend={'Fetching relations...'} />
      ) : (
        <div>
          <SimpleSelect
            ...
            {baseEntities.map((entity, i) => (
              <div className={'parent-entity'} key={entity._id}>
                <div className={'entity-name'}>
                  <PinkCheckbox
                    checked={_.get(checkedEntities[i], 'checked', false)}
                    onChange={() => handleCheck(checkedEntities[i])}
                  />
                  {entity.entityName}
                  <div className={'toggle'}>
                    {collapsed[i] ? (
                      <ExpandMoreIcon onClick={() => toggle(i)} />
                    ) : (
                ...
                <div className={clsx('entity-children', collapsed[i] && 'collapsed')}>
                  {_.isEmpty(getFilteredRelated(entity)) ? (
                    ...
                    getFilteredRelated(entity).map(r => (
                      <div
                        key={r._id}
                      ...
                      >
                        <div className={'child-text'}>
                          {r.removed ? (
                            <LinkOff className={'link-icon'} />
                          ) : (
                            <LinkTwoToneIcon className={'link-icon'} />
                          )}
                        ...
                        {!r.removed ? (
                          <div
                            ...
                            onClick={() => removeRelated({ related: r, entityId: entity._id })}
                          >
                          ...
                          <div
                            className="delete-button"
                            onClick={() => recoverRelated({ related: r, entityId: entity._id })}
                          >

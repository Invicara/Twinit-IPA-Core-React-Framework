...
export default class EntityCollectionModal extends React.Component {
  ...
  async componentDidMount() {
    if (
      !this.props.action.script ||
      !this.props.action.component.scripts ||
      !this.props.action.component.scripts.getCollectionTypes ||
      !this.props.action.component.scripts.getCollectionNames
    ) {
  ...
  async componentDidUpdate(prevProps, prevState) {
    if (this.state.modalOpen !== this.context.ifefModalOpen) {
      ...
      if (this.context.ifefModalOpen) {
        ...
        let isSingleEntity =
          this.props.entity &&
          typeof this.props.entity === 'object' &&
          !Array.isArray(this.props.entity);
  ...
  getFilterInfo = () => {
    ...
    if (value) return { query: { 'properties.Type.val': value } };
    else return { query: null };
  };

  isValidCollection = collection => {
    return (
      !!collection.type && !collection.type !== '' && !!collection.name && !collection.name !== ''
    );
  };

  onChange = async (att, value) => {
    if (att === 'type') {
      if (this.getSelectedVal(value)) this.setState({ selectedType: value });
      ...
      if (
        this.isValidCollection(newCollToAdd) &&
        !_.find(this.state.selectedCollections, newCollToAdd)
      ) {
  ...
  onSave = async () => {
    ...
    if (this.props.action.doEntityAction && !!this.state.selectedCollections.length) {
      ...
      if (result?.success) {
        this.context.ifefShowModal(false);
        if (this.props.action.onSuccess)
          this.props.action.onSuccess(
            this.props.action.type,
            result.entity ? result.entity : this.props.entity,
            result
          );
      } else {
        console.error(result.message);
        let error = <div className="entity-modal-error">{result.message}</div>;

        this.setState({ working: false, error: error });
        if (this.props.action.onError)
          this.props.action.onError(this.props.action.type, result, this.props.entity);
      }
    }
    ...
    let title = (
      <span>
        <i
          ...
          className={this.props.action.component.icon || 'icofont-cubes'}
        ></i>{' '}
        Add to Collection
      </span>
    );
    let body = this.state.showBody ? (
      <div className="entity-collection-modal-body">
        ...
        <div className="selected-collections">
          ...
          <div className="selected-collections-display">
            {this.state.selectedCollections.map(coll => (
              <div className="selected-collections-display-row" key={coll.type + ':' + coll.name}>
                <span className="selected-collection-name-combo">
                  {coll.type + ': ' + coll.name}
                </span>
                <span
                  className="clear-selected-collection"
                  onClick={() => this.removeCollection(coll)}
                >
      ...
      <div>
        Nothing selected to add to Collections. Please close the dialog and select some items to add
        to Collections.
      </div>
    );

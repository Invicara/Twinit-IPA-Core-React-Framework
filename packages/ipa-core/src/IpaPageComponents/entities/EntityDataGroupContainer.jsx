...
const EntityDataGroupContainer = props => {
  const getArrayWithoutHiddenData = config => {
    ...
    return props.data.map(e =>
      Array.isArray(e) ? e.filter((o, index) => !indexesToHide.includes(index)) : e
    );
  };
...
export default compose(connect(mapStateToProps, mapDispatchToProps))(EntityDataGroupContainer);

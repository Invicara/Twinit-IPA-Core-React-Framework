...
const withEntitySearch = WrappedComponent => {
  const EntitySearchHOC = class extends React.Component {
    constructor(props) {
      super(props);
      let queryParamsPerEntityType = this.initQueryParamsValues(
        props,
        props.currentEntityType.singular
      );
      this.state = {
        isPageLoading: true,
    ...
    setQueryParams = (queryParamsPartial, callback) => {
      ...
      let queryParamsPerEntityType = {
        ...this.state.queryParamsPerEntityType,
        [entityType]: merged,
      };
    ...
    initialFetchFromQuery = async () => {
      ...
      if (queryParams !== undefined) {
        ...
        if (
          queryParams.query &&
          _.includes(this.props.allowedEntityTypes, queryParams.entityType) &&
          (!queryParams.senderEntityType ||
          ...
          if (!selector)
            selector = queryEntityConfig.selectors.find(s => s.id === queryParams.query.id);
          ...
          if (
            ...
            Array.isArray(queryParams.query.value)
          ) {
        ...
        //TODO add a config validator on load
          ...
          console.warn(
            'Incompatible entity type provided in query params got:',
            queryParams.entityType,
            'but expected one of ',
            this.props.allowedEntityTypes
          );
        } else {
          !_.isEmpty(queryParams) &&
            console.warn('Ignoring query:', queryParams, 'for entity: ', this.props.entitySingular);
        }
      }
    };
    ...
    doEntityAction = async (action, entityInfo, type) => {
      ...
      if (!Object.keys(actions).includes(action)) {
        ...
        if (navConfig.script) {
          ...
          else selectedEntities = this.props.filteredEntities;

          let result = await ScriptHelper.executeScript(navConfig.script, {
          ...
          let entityType = navConfig.scriptResultType
            ? navConfig.scriptResultType
            : this.props.entitySingular;
          ...
          selectionInfo['queryParams'] = {
            ...this.state.queryParamsPerEntityType[this.props.entitySingular],
            selector: undefined,
          };
          selectionInfo['entityType'] =
            (typeof type === 'string' ? type : type?.singular) || this.props.entitySingular;
        ...
        FileHelpers.downloadDocuments(
          Array.isArray(entityInfo.original) ? entityInfo.original : [entityInfo.original]
        );
        return { success: true };
    ...
    getFetcher =
      (script, originalSender, runScriptOptions) =>
      async (selector, value, initialPageLoad, onInitialFetchComplete) => {
        ...
        const fetchedEntities = await this.props.fetchEntities(
          script,
          selector,
          value,
          runScriptOptions,

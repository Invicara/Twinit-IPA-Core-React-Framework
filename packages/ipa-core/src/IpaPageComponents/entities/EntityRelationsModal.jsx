...
export default class EntityRelationsModal extends React.Component {
  ...
  async componentDidMount() {
    ...
    if (
      ...
      !this.props.action.component.scripts.getRelatedEntities
    )
  ...
  removeRelation = relatedEntity => {
    ScriptHelper.executeScript(this.props.action.script, {
      removeEntities: [relatedEntity],
      removeFrom: [this.props.entity],
      entityType: this.state.currentType.value,
    }).then(result => {
      if (result?.success) {
        this.getEntitiesForCollection();
      } else {
  ...
  onCancel = async () => {
    if (this.props.action.onSuccess)
      this.props.action.onSuccess(this.props.action.type, this.props.entity, this.props.entity);
  ...
  render() {
    let title = (
      <span>
        <i style={{ fontSize: '1.5em' }} className="icofont-link"></i>{' '}
        {this.props.action.component.title ? this.props.action.component.title : 'Manage Relations'}
      </span>
    );
    let body = (
      <div className="entity-relations-modal-body">
        <Select
        ...
        <div className="relations">
          ...
          {!!this.state.currentRelatedEntities?.length &&
            this.state.currentRelatedEntities?.map(re => (
              <div key={re._id} className="selected-relations-display-row">
                <span className="remove-from-collection" onClick={() => this.removeRelation(re)}>

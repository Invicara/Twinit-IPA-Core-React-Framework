...
const withGenericPage = (PageComponent, optionalProps = {}) => {
  class GenericPage extends React.Component {
    ...
    componentDidUpdate(prevProps) {
      ...
      if (
        this.state.project &&
        this.state.project._id !== this.props.selectedItems.selectedProject._id
      ) {
        this.setState({
          project: this.props.selectedItems.selectedProject,
          userConfig: this.props.selectedItems.userConfig,
      ...
      if (
        ...
        this.state.userConfig._id !== this.props.userConfig._id
      )
        this.props.history.push('/');
    }
    ...
    async _loadPageData() {
      ...
      try {
        handler = await produce(this.props.actions.getCurrentHandler(), async handler => {
          ...
          if (
            handler.config &&
            !handler.config.selectBy &&
            handler.config.type &&
            this.props.userConfig.entitySelectConfig
          ) {
            if (
              !Array.isArray(handler.config.type) &&
              this.props.userConfig.entitySelectConfig[handler.config.type.singular]
            ) {
              //if the page is expecting only one selectBy config
              handler.config = {
                ...handler.config,
                selectBy: this.props.userConfig.entitySelectConfig[handler.config.type.singular],
              };
            } else {
              ...
              handler.config = {
                ...
                selectBy: _.fromPairs(
                  handler.config.type
                    .filter(t => !!this.props.userConfig.entitySelectConfig[t.singular])
          ...
          if (hasDefaultConfig && handlerHasConfig) {
            if (expectsMultipleDataConfig) {
              ...
              const defaultData =
                entityType && this.props.userConfig.entityDataConfig[entityType.singular];
              let data = produce(defaultData || {}, function (data) {
                //merge handler config into default data, so treat as full override
                _.merge(data, handlerData || {});
              });
    ...
    setQueryParams = partial =>
      this.setState({ queryParams: { ...this.state.queryParams, ...partial } });

    //provide pageComponents with a navigate function to place query criteria in the url to pass to other pages
    onNavigateOut =
      (userConfig, genericQueryParams, validator, history) =>
      (destinationHandler, selectionInfo, options = {}) => {
        /*
        ...
        if (!destinationHandler || !Object.keys(userConfig.handlers).includes(destinationHandler)) {
          console.error(
            'Attempting to navigate without a valid destination handler:',
            destinationHandler
          );
          return false;
        }
        ...
        if (!selectionInfo && !genericQueryParams.query) {
          ...
          if (query.query && query.query.value && !query.query.id && query.query.id !== 0)
            query.query.value = Array.isArray(query.query.value)
              ? query.query.value.join(',')
              : query.query.value;
          if (
            selectionInfo &&
            selectionInfo.selectedEntities &&
            selectionInfo.selectedEntities.length > 0
          ) {
            ...
            Array.isArray(query.selectedEntities) &&
    ...
    toolbar = () => {
      return (
        this.state.handler.toolbar && (
          <Container maxWidth="xl">
            <Toolbar disableGutters>
              {/*breadcrumbs to the left*/}
              <Box sx={{ flexGrow: 1 }}>
                {this.state.handler.toolbar.breadcrumbButtons &&
                  this.state.handler.toolbar.breadcrumbButtons.map(b => (
                    <GenericMatButton key={b.text} {...b.props}>
                      {b.text}
                    </GenericMatButton>
                  ))}
              </Box>
              ...
              <Box sx={{ flexGrow: 0 }}>
                {this.state.handler.toolbar.actionButtons &&
                  this.state.handler.toolbar.actionButtons.map(b => (
                    <GenericMatButton key={b.text} {...b.props}>
                      {b.text}
                    </GenericMatButton>
                  ))}
                {this.state.handler.toolbar.pagination &&
                  this.state.handler.pagination.toolbar.hasPrevious && (
                    <GenericMatButton>Prev</GenericMatButton>
                  )}
                {this.state.handler.toolbar.pagination &&
                  this.state.handler.pagination.toolbar.hasNext && (
                    <GenericMatButton>Next</GenericMatButton>
                  )}
              </Box>
            </Toolbar>
          </Container>
    ...
    isNestedDetailPage = () =>
      optionalProps && optionalProps.detailPage && optionalProps.detailPage.nested;

    withPageLayout = body => (
      <div className="page">
        <div className="generic-page-body">{body}</div>
      </div>
    );

    renderGenericPage = genericPageContext => {
      return this.isNestedDetailPage()
        ? this.body(genericPageContext)
        : this.withPageLayout(this.body(genericPageContext));
    };

    render() {
      const onNavigate = this.onNavigateOut(
      ...
      return (
        <GenericPageContext.Provider value={genericPageContext}>

...
class DashboardView extends React.Component {
  ...
  generatePanels = async () => {
    ...
    if (config.layout === 'fullpage') panels = await this.getComponent(config);
    else if (config.layout === 'grid') {
      let panelEntries = Object.entries(config.panels);

      panels = await Promise.all(
        panelEntries.map(async ([name, panelConfig]) => {
  ...
  getComponent = async config => {
    ...
    if (script) {
      componentInfo = await ScriptHelper.executeScript(script, {
        reactorInfo: this.state.reactorInfo,
      });
      if (componentInfo && componentInfo.component)
        Component = await this.props.getDashboardComponent(componentInfo.component);
    }
  ...
  render() {
    ...
    return (
      <div>
        {!!this.state.preloadImageUrls.length && (
          <div id="hiddenimages" style={{ display: 'none' }}>
            {this.state.preloadImageUrls.map(imageurl => (
        ...
        {config.headerInfo && (
          <div className="dashboard-header">
            ...
            {config.headerInfo.navButton && (
              <div className="nav-button">
                <GenericMatButton
                  customClasses="main-button"
                  onClick={e => this.doAction(config.headerInfo.navButton.action)}
                >
                  {config.headerInfo.navButton.title}
                </GenericMatButton>
              </div>
            )}
          </div>
...
export default compose(connect(mapStateToProps, mapDispatchToProps))(DashboardView);

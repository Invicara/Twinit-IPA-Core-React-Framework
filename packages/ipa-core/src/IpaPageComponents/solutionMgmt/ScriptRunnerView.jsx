...
class ScriptRunnerView extends React.Component {
  ...
  async _loadAsyncData() {
    ...
    if (handler.config && handler.config.allowScriptInput && scripts && scripts.length > 0) {
      ...
      if (
        localStorage.ipaScriptRunnerLocalScripts &&
        localStorage.ipaScriptRunnerLocalScripts.length
      ) {
      ...
      this.setState(
        { parsedScript: allParsedScripts, scripts: scripts, selectedScript: scripts[0], operators },
        this.getScriptText
      );

      if (localStorage.ipaScriptRunnerConvertSetq) this.setState({ convertSetq: true });
      this.readVars();
      ...
      if (helpLinks && helpLinks.length) this.setState({ helpLinks: helpLinks });
    } else
      this.setState({
        scripts: scripts,
        ...
        if (def.hasOwnProperty('$defscript')) {
          ...
          if (namedScript.hasOwnProperty(this.state.selectedScript.script)) {
            ...
            if (this.state.selectedScript.defaultInput) {
              this.setState({
                scriptInput: JSON.stringify(this.state.selectedScript.defaultInput, null, 3),
              });
            } else this.setState({ scriptInput: '' });
          }
        }
      });

      if (!defaultScript) this.setState({ scriptJSON: 'Script Not Found', scriptError: true });
    } else {
  ...
  clearVars(v) {
    ...
    } else this.setState({ variables: {} }, this.writeVars);
  }
  ...
  handleScriptSelect(option) {
    ...
    else this.setState({ selectedScript: option.value });
  }
    ...
    if (this.props.handler.config && this.props.handler.config.allowScriptInput) {
      ...
      if (this.state.hasBreakLine) {
        ...
        if (!this.isValidBreakPoint(this.state.editor, lineInfo))
          ...
          let range = this.state.editor.getRange(
            { line: 0, ch: 0 },
  ...
  onInputChange(editor, data, value, which) {
    ...
    if (which === 'input') this.setState({ scriptInput: value, inputError: inputError });
    else {
  ...
  toggleInputs(input) {
    if (input === 'input') this.setState({ showInput: !this.state.showInput });
    else if (input === 'script') this.setState({ showScript: !this.state.showScript });
    else this.setState({ showOutput: !this.state.showOutput });
  }

  toggleConvertSetq() {
    if (this.state.convertSetq) localStorage.removeItem('ipaScriptRunnerConvertSetq');
    else localStorage.setItem('ipaScriptRunnerConvertSetq', true);
  ...
  addNewScript() {
    ...
    if (!localStorage.ipaScriptRunnerLocalScripts)
      ...
      localStorage.setItem(
        'ipaScriptRunnerLocalScripts',
        localStorage.ipaScriptRunnerLocalScripts + ',' + newName
      );
  ...
  removeLocalScript(e, scriptname) {
    ...
    else this.setState({ scripts: scripts });
  }
  ...
  handleScriptGutterClick(editor, lineNum, gutter, event) {
    if (gutter === 'CodeMirror-breakpoint') {
      ...
      if (
        ...
        lineInfo.gutterMarkers.hasOwnProperty('CodeMirror-breakpoint')
      ) {
  ...
  getOperatorHints(editor, option) {
    return new Promise(resolve => {
      ...
      let completions = this.state.operators.filter(o =>
        o.toLowerCase().startsWith(word.toLowerCase())
      );
  ...
  searchOperators() {
    let searchedOperators = this.state.operators.filter(o =>
      o.toLowerCase().includes(this.state.operatorSearchTerm.toLowerCase())
    );
    ...
    return (
      <div style={{ display: 'flex' }}>
          ...
          <StackableDrawer level={2} iconKey="far fa-file-code" isDrawerOpen={false}>
            <div style={{ marginTop: '10px', marginLeft: '60px' }}>
              ...
              <GenericMatButton
                ...
                styles={{ marginLeft: '110px', marginRight: '10px', marginTop: '5px' }}
            ...
            <hr />
            <div style={{ fontWeight: 'bold', marginTop: '20px', marginLeft: '60px' }}>
              Local Scripts
            </div>
            <div style={{ marginTop: '40px', marginLeft: '60px' }}>
              {this.state.scripts
                .filter(scriptoption => scriptoption.local)
                .map((scriptoption, index) => (
                  <div style={{ marginBottom: '10px' }} key={index}>
                    <a
                      ...
                      onClick={e => this.removeLocalScript(e, scriptoption.script)}
                    >
                      <i title="Delete Local Script" className="icon ion-android-cancel"></i>
                    </a>{' '}
                    {scriptoption.script}
                  </div>
                ))}
            ...
            <div style={{ marginTop: '10px', marginLeft: '60px' }}>
              ...
              <div style={{ marginTop: '10px' }}>
                ...
                <label style={{ marginLeft: '5px' }}>
                  Convert all $let to $setq when executing
                </label>
              </div>
              <hr />
              <div
              ...
              <div style={{ cursor: 'pointer' }} onClick={() => this.clearVars('all')}>
                <span style={{ fontSize: '18px' }}>
                  <i title="Unwatch All" className="fas fa-eye-slash"></i>
                </span>
                  ...
                  <div key={v} style={{ marginBottom: '10px' }}>
                    <div style={{ display: 'inline-flex' }}>
                      <div
                        ...
                        onClick={() => this.clearVars(v)}
                      >
                        <i title="Unwatch" className="fas fa-eye-slash"></i>
                      </div>
                      <div
                        style={{ fontSize: '18px', cursor: 'pointer', marginRight: '10px' }}
                        onClick={() => this.clearVarValue(v)}
                      >
                        <i title="Clear Value" className="fas fa-times-circle"></i>
                      </div>
        ...
        {this.props.handler.config &&
          this.props.handler.config.allowScriptInput &&
          this.state.helpLinks.length > 0 && (
            <StackableDrawer level={4} iconKey="fas fa-dollar-sign" isDrawerOpen={false}>
              <div style={{ marginTop: '10px', marginLeft: '60px' }}>
                <div style={{ display: 'inline-flex', alignItems: 'center' }}>
                  ...
                  >
                    <i title="Clear Search" className="fas fa-times-circle"></i>
                  </div>
                  <GenericMatButton
                    disabled={this.state.isRunning}
                    styles={{ marginLeft: '10px', marginRight: '10px' }}
              ...
              <div style={{ marginTop: '20px', marginLeft: '60px' }}>
                {!this.state.searchedOperators
                  ? this.state.operators.map((op, index) => (
                      <div style={{ marginBottom: '10px' }} key={index}>
                        {op}
                      </div>
                    ))
                  : this.state.searchedOperators.map((op, index) => (
                      <div style={{ marginBottom: '10px' }} key={index}>
                        {op}
                      </div>
                    ))}
        ...
        {this.props.handler.config &&
          ...
          this.state.helpLinks.length > 0 && (
            <StackableDrawer level={5} iconKey="fas fa-question" isDrawerOpen={false}>
              ...
              <div style={{ marginTop: '40px', marginLeft: '60px' }}>
                {this.state.helpLinks.map((link, index) => (
                  <div style={{ marginBottom: '10px' }} key={index}>
          ...
          <div style={{ padding: '40px', marginTop: '20px' }}>
            <div style={{ marginLeft: '15%', marginRight: '15%' }}>
              <div style={{ display: 'flex', alignItems: 'center' }}>
                ...
                {this.state.isRunning && (
                  <span style={{ fontSize: '16px' }}>Running {this.state.dots}</span>
                )}
                {this.props.handler.config && this.props.handler.config.allowScriptInput && (
                  <GenericMatButton
                    onClick={() => this.toggleInputs('input')}
                    disabled={this.state.isRunning}
                    styles={{ marginLeft: '20px', marginRight: '10px' }}
                  >
                ...
                {this.props.handler.config && this.props.handler.config.allowScriptInput && (
                  <GenericMatButton
                    onClick={() => this.toggleInputs('script')}
                    disabled={this.state.isRunning}
                    styles={{ marginLeft: '10px', marginRight: '10px' }}
                  >
                ...
                {this.props.handler.config && this.props.handler.config.allowScriptInput && (
                  <GenericMatButton
                    onClick={() => this.toggleInputs('output')}
                    disabled={this.state.isRunning}
                        ...
                        <div className="script-input" style={{ marginTop: '20px', width: '100%' }}>
                          <CodeMirror
                            ...
                            onBeforeChange={(editor, data, value) =>
                              this.onInputChange(editor, data, value, 'input')
                            }
                          />
                        </div>
                        {this.state.inputError && (
                          <div style={{ color: 'red' }}>Input is not in JSON format</div>
                        )}
                      </div>
                    )}
                    {this.state.showScript && (
                      <div>
                        {this.state.scriptError && (
                          <div style={{ color: 'red' }}>Script is not in correct format</div>
                        )}
                        <div className="script-script" style={{ marginTop: '20px', width: '100%' }}>
                          <CodeMirror
                            ...
                            onBeforeChange={(editor, data, value) =>
                              this.onInputChange(editor, data, value, 'script')
                            }
              ...
              {this.state.showOutput &&
                this.state.results.map((res, index) => (
                  <div key={index}>
                        ...
                        {this.state.results.length > 0 && (
                          <div style={{ textAlign: 'right', color: 'red' }}>
                            ...
                            <a
                              ...
                              onClick={e => this.deleteResult(e)}
                            >
                              <i title="Delete All" className="icon ion-android-cancel"></i>
                            </a>
                          </div>
                        )}
                      </div>
                    ...
                    <div className="mbsc-row mbsc-align-items-center">
                      <div className="mbsc-col-md-6 mbsc-col-6" style={{ textAlign: 'left' }}>
                        ...
                        {!res.collapsed && (
                          <a
                            ...
                            onClick={e => this.toggleDisplay(e, index)}
                          >
                            {res.disp ? 'Raw' : 'Pretty'}
                          </a>
                        )}
                      ...
                      <div className="mbsc-col-md-6 mbsc-col-6" style={{ textAlign: 'right' }}>
                        <a
                          ...
                          onClick={e => this.toggleCollapsed(e, index)}
                        >
                          {res.collapsed ? 'Show' : 'Hide'}
                        </a>
                        <a
                          href="#"
                          style={{
                          ...
                          onClick={e => this.deleteResult(e, index)}
                        >
                          <i title="Delete" className="icon ion-android-cancel"></i>
                        </a>
                      </div>
                      <div
                    ...
                    {!res.collapsed && (
                      <div style={{ marginTop: '10px', userSelect: 'text' }}>

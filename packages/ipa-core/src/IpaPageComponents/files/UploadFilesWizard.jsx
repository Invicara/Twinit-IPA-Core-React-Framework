...
const UploadFilesWizard = ({
  ...
  const [uploadContainer, setUploadContainer] = useState(
    selectedItems.selectedProject.rootContainer
  );
  ...
  useEffect(() => {
    async function getUploadContainer() {
      let newContainer = await ScriptHelper.executeScript(config.uploadContainer.script, {
  ...
  const startUpload = async () => {
    ...
    const errors = await uploadFiles(
      uploadContainer,
      config.scripts.processUploadFile,
      config.scripts.postprocessFiles,
      5,
  ...
  const handleFileChange = config.readonly
    ? _.noop
    : (files, field, newValue) => {
        const normalizedField = Array.isArray(field) ? field.join(',') : field;
        const fieldCandidates = Array.isArray(field)
        ...
        const affectsContainer =
          containerAffectingAttributes &&
          containerAffectingAttributes.length > 0 &&
          containerAffectingAttributes.some(attr => fieldCandidates.includes(attr));

        const shouldRecomputeVersion =
          !!config.enableDynamicVersioning &&
          !!config.scripts?.getFileContainer &&
          affectsContainer;
  ...
  const buildReport = () => {
    const headers = [
      'Name',
      ...columnConfig.map(col => `${col.displayAs}${col.required ? '*' : ''}`),
    ];
    const rows = files.map(file => [
      file.name,
      ...columnConfig.map(col => getValue(file.fileAttributes[col.name])),
    ]);
  ...
  const steps = [
    {
      name: 'Add Files',
      component: config.scripts.seedAttributes ? (
        <SeedAttributes onClick={seedAttributes} />
      ) : (
        <div />
      ),
    },
    {
      ...
      component: columnConfig ? (
        <FileTable
          ...
          LinkedSelectValues={LinkedSelectValues}
        />
      ) : (
        'Loading...'
      ),
    ...
    },
    {
      name: 'Review',
      component: columnConfig ? (
        <FileTable columns={columnConfig} files={files} onFileChange={handleFileChange} readonly />
      ) : (
        'Loading...'
      ),
      buttons: WizardButtons({
...
export default compose(connect(mapStateToProps, mapDispatchToProps))(UploadFilesWizard);

...
const DocumentTable = props => {
  ...
  useEffect(() => {
    if (sort && !_.isEmpty(sort.currentColumn)) {
      let newDocuments = _.sortBy(props.documents, a => {
        if (_.includes(sort.currentColumn, 'properties.'))
          return a.documentData[`${sort.currentColumn}.val`];
  ...
  const toogleVersionDrop = fileId => {
    if (
      !document
        .querySelector(`.v${fileId}`)
        .classList.contains('document-table__version_dropdown_show')
    )
      document.querySelector(`.v${fileId}`).classList.add('document-table__version_dropdown_show');
    else
      document
        .querySelector(`.v${fileId}`)
        .classList.remove('document-table__version_dropdown_show');
  };
  const setSelectedVersion = (version, doc) => {
    ...
    if (!doc.currentVersion.includes(version)) {
      newDocuments
        .find(d => d.documentData._fileId === doc.documentData._fileId)
      ...
      if (selectedDocIndex === -1) {
        ...
        if (!newSelectedDocumentsIds.includes(doc.documentData._fileId))
          newSelectedDocumentsIds.push(doc.documentData._fileId);
      }
    } else {
      _.pull(
        newDocuments.find(d => d.documentData._fileId === doc.documentData._fileId)?.currentVersion,
        version
      );
      if (selectedDocIndex != -1) {
        if (!_.isEmpty(selectedDocumentsIds)) newSelectedDocumentsIds = [...selectedDocumentsIds];
        if (newSelectedDocumentsIds.includes(doc.documentData._fileId))
          _.pull(newSelectedDocumentsIds, doc.documentData._fileId);
      }
    }
  ...
  const selectAllVersions = doc => {
    ...
    doc.documentData.versions.forEach(v => {
      if (
        !newDocuments
          .find(d => d.documentData._fileId === doc.documentData._fileId)
          ?.currentVersion.includes(v)
      )
        newDocuments
          .find(d => d.documentData._fileId === doc.documentData._fileId)
          ?.currentVersion.push(v);
    });
    setDocuments(newDocuments);
  };
  ...
  const deselectAllVersions = doc => {
    ...
    if (doc.documentData.versions.length > 1) {
      doc.documentData.versions.forEach(v => {
        _.pull(
          newDocuments.find(d => d.documentData._fileId === doc.documentData._fileId)
            ?.currentVersion,
          v
        );
      });
  ...
  return (
    <div className="document-table">
        ...
        {props.actions
          ...
          .map(action => {
            ...
            if (action.bulk.component) {
              return (
                <action.bulk.component key={action.name} {...defaultProps} {...action.bulk.props} />
              );
            }
            return (
              <div
                key={action.name}
                className={`document-table__action document-table__action${selectedDocumentsIds.length > 0 ? '_enabled' : '_disabled'}`}
              >
      ...
      {documents.length > 0 ? (
        <div className={`document-table__count`}>Showing {documents.length} files</div>
      ) : null}
      {documents.length > 0 ? (
        <table className="document-table__table">
          <tr className="document-table__row document-table__row--header">
            ...
            <th className="document-table__col document-table__col--header document-table__col document-table__col--actions"></th>
            <th className="document-table__col document-table__col--header">
              <div>
                Version
                <span className={`document-table_lock`}>
                  <i
                    className={`fas ${_.includes(props?.tableConfig?.lockedColumns, 'Version') ? 'fa-lock' : ''}`}
                  ></i>
                </span>
              </div>
            </th>
            <th className="document-table__col document-table__col--filename">
              <div>
                Filename
                <span
                  className={`document-table__filter${sort ? (sort.currentColumn === 'name' ? '_active' : '') : ''}`}
                >
                  <i
                    onClick={() => sortBy('name', sort ? sort.isDescending : true)}
                    className={`fas ${sort ? (sort.isDescending ? 'fa-sort-amount-up' : 'fa-sort-amount-down') : 'fa-sort-amount-down'}`}
                  ></i>
                </span>
              </div>
            </th>
            {props.tableConfig.columns
              ...
              .map((column, index, array) => {
                return (
                  <th
                    key={column.accessor}
                    className="document-table__col document-table__col--header"
                  >
                    <div>
                      {column.name}
                      <span className={`document-table_lock`}>
                        <i
                          className={`fas ${_.includes(props?.tableConfig?.lockedColumns, column.name) ? 'fa-lock' : ''}`}
                        ></i>
                      </span>
                      <span
                        className={`document-table__filter${sort ? (sort.currentColumn === column.accessor ? '_active' : '') : ''}`}
                      >
                        <i
                          onClick={() => sortBy(column.accessor, sort ? sort.isDescending : true)}
                          className={`fas ${sort ? (sort.isDescending ? 'fa-sort-amount-up' : 'fa-sort-amount-down') : 'fa-sort-amount-down'}`}
                        ></i>
                      </span>
                      {array.length === index + 1 ? (
                        <span className="document-table__reorder_button">
                          <i
                            onClick={() =>
                              OpenReorderModal(
                                props.tableConfig.columns,
                                props.tableConfig.onColumnsChange,
                                props?.tableConfig?.lockedColumns
                              )
                            }
          ...
          {documents.map((doc, index) => {
            ...
            let checked =
              selectedDocIndex != -1 &&
              (doc.documentData.versions.length > 1
                ? doc.currentVersion.length === doc.documentData.versions.length
                : true);
            let partialChecked =
              doc.currentVersion.length < doc.documentData.versions.length &&
              doc.currentVersion.length != 0;
            let versions = _.intersection(doc.documentData.versions, doc.currentVersion);
            return (
              <tr key={doc.documentData.fileId} className="document-table__row">
                ...
                <td className="document-table__col document-table__col--actions">
                  <div className="document-table__actions document-table__actions--row">
                    {_.values(getDocActions(index)).map(action => {
                      ...
                      if (action.component) {
                        return (
                          <action.component key={action.name} {...defaultProps} {...action.props} />
                        );
                      }
                      return (
                        <span key={action.name} className={`document-table__action-button`}>
                          <Tooltip title={defaultProps.title}>
                            <i
                              className={`${defaultProps.icon} ${disabled.toString()}`}
                ...
                <td className="document-table__col document-table__col--version">
                  <div>
                    <div className={`document-table__version_display`}>
                      {versions.map((v, i) => {
                        return `${v.versionNumber}${versions.length > 1 && versions.length != i + 1 ? ',' : ''}`;
                      })}
                      <span
                        className={`${doc.documentData.versions.length > 1 ? 'document-table_dropdown_icon_enabled' : 'document-table_dropdown_icon_disabled'}`}
                      >
                        <i
                          className={`fas fa-caret-down`}
                          onClick={() => toogleVersionDrop(fileId)}
                        ></i>
                      </span>
                    </div>
                    <div className={`document-table__version_dropdown v${fileId}`}>
                      {doc.documentData.versions.length > 1
                        ? doc.documentData.versions.map(v => {
                            let vdate;
                            ...
                            return (
                              <div key={v.versionNumber}>
                                <PinkCheckbox
                                  onChange={() => setSelectedVersion(v, doc)}
                                  checked={doc.currentVersion?.includes(v)}
                                />
                                {v.versionNumber}{' '}
                                {vdate
                                  ? vdate.format('DD/MM/YYYY kk:mm:ss a')
                                  : date.format('DD/MM/YYYY kk:mm:ss a')}
                              </div>
                            );
                          })
                        : null}
                ...
                {props.tableConfig.columns.map(column => {
                  if (column.active) {
                    ...
                    return (
                      <td key={column.accessor} className="document-table__col">
                        {value}
                      </td>
                    );
                  }

...
export const UploadFilesWizardSteps = ({
  steps,
  selectedStep,
  addFiles,
  startUpload,
  ...
  const assetList = (
    <ul>
      {associatedEntities.map(entity => (
        <li key={entity._id} className={'file-asset'}>
          {entity['Entity Name']}
        </li>
      ))}
    </ul>
  ...
  return (
    <div className="upload-files-wizard-root with-buttons">
      {isloading ? (
        <LinearProgress
          sx={{
            height: '4px',
      ...
      <div className={clsx('wizard-panel', panelOpen && 'wizard-panel-expanded')}>
        <WizardTimeline steps={steps.map(({ name }) => name)} selectedStep={selectedStep} />
        {
          <ChooseFiles
      ...
      <div className={clsx('files-area', panelOpen && 'extra-margin')}>
        {hasEntities && (
          <div className="files-area-title">
      ...
      {!_.isEmpty(rejectedFiles) && (
        <div className={'rejected-files'}>
          {!hideDefaultError &&
            `*The following files were rejected and will not be uploaded: ${rejectedFiles.map(f => f.name).join(' ,')}`}
      ...
      {Buttons && (
        <Buttons
          optionsOverride={
            selectedStep === 2 && hasEntities ? { onPrimaryClick: () => setDialogOpen(true) } : {}
          }
        />
      )}
      <Dialog onClose={closeDialog} open={dialogOpen}>
        <div className={'dialog-content with-buttons'}>
          ...
          <div className={'dialog-body'}>
            Are you sure you want to upload these files with your selected metadata and associate
            them with the following:
          </div>
          {assetList}
          <div className={'dialog-actions'}>
            <GenericMatButton customClasses="main-button" onClick={handleUpload}>
              <i className="fas fa-upload" /> Upload
            </GenericMatButton>
            <GenericMatButton customClasses="cancel-button" onClick={closeDialog}>
              Cancel
            </GenericMatButton>
          </div>
        </div>

...
const fallbackControlBuilder = async column => {
  throw new Error(
    `Error in file column config for ${column.name}: Unknown control for string query ${column.query}`
  );
};
...
const controlBuilders = {
  '<<CREATABLE_SCRIPTED_SELECTS>>': async column => {
    ...
    if (values) {
      selectValues = _.mapValues(values, options => options?.sort((a, b) => a.localeCompare(b)));
    } else {
      selectValues = {};
    }
    if (!column.script)
      throw new Error(
        `Error in file column config for ${column.name}: <<CREATABLE_SCRIPTED_SELECTS>> require a script to populate select options`
      );
  ...
  '<<TEXT>>': () => (value, onChange) => (
    <FileTableInput initialValue={value} onChange={onChange} />
  ),
  '<<SIMPLE_SELECT>>': async column => {
    if (!column.script)
      throw new Error(
        `Error in file column config for ${column.name}: <<SIMPLE_SELECT>> requires a script to populate select options`
      );
    const values = _.values(await ScriptHelper.executeScript(column.script))[0];
    return (value, onChange) => (
      <FileTableSelect value={value} options={values} onChange={onChange} />
    );
  },
};
...
const buildConfig = displayNames =>
  produce(async column => {
    if (typeof column.query === 'object') {
      ...
      throw new Error(
        `Error in file column config for ${column.name}: Requires a query (either object or string) to populate select options`
      );
    }
    const columnNameArray = typeof column.name === 'string' ? [column.name] : column.name;
    column.displayAs = displayNames
      .filter(({ prop }) => _.includes(columnNameArray, prop))
      .map(({ dName }) => dName)
      .join(' & ');

...
import {
  RadioButtonChecked as CheckedCircle,
...
export const RoundFullCheckbox = ({ ...props }) => (
  <Checkbox
...
export const FileUploadTable = ({ files: inputFiles }) => {
  const getType = file => {
    ...
    return (
      ...
      (inferredExtension && inferredExtension.toUpperCase()) ||
      'UNKNOWN'
    );
  };
        ...
        <tbody>
          {inputFiles.map((file, i) => (
            <tr
              ...
              style={isPending(file) ? { color: '#B2B2B2' } : {}}
            >
              <td>
              ...
              <td>
                {isInProgress(file)
                  ? `${formatBytes(file.bytesUploaded)} of ${formatBytes(getBlob(file).size)}`
                  : formatBytes(getBlob(file).size)}
              </td>

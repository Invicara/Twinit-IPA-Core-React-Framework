...
export const WizardTimeline = ({ steps, selectedStep }) => {
  ...
  return (
    <div className="wizard-timeline-root">
        ...
        return (
          <div className="step" key={stepNumber}>
            <div className="step-symbol">
              {!isFirst(stepNumber) ? (
                <div className={clsx('step-cord', isSelected(stepNumber) && 'selected')} />
              ) : null}
              <div
                className={clsx(
                  'step-bubble',
                  isSelected(stepNumber) && 'selected',
                  isPrevious(stepNumber) && 'previous'
                )}
              >
                {stepNumber}
              </div>
              {!isLast(stepNumber) ? (
                <div className={clsx('step-cord', isSelected(stepNumber + 1) && 'selected')} />
              ) : null}
            </div>
            <div
              className={clsx(
                'step-legend',
                isFirst(stepNumber) && 'first',
                isLast(stepNumber) && 'last',
                isSelected(stepNumber) && 'selected',
                isPrevious(stepNumber) && 'previous'
              )}
            >

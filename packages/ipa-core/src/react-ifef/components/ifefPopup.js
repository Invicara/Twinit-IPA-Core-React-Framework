...
class IfefPopup extends React.Component {
  ...
  render() {
    let ifefPopup = this.props.ifefPopup;

    let willMount = true;
    if (_.isEmpty(ifefPopup)) willMount = false;

    let title = ifefPopup.title;
    let subTitle = ifefPopup.subTitle;
    let template = ifefPopup.template;
    let buttons = ifefPopup.buttons;
    let cancel = ifefPopup.cancel;
    let popupType = ifefPopup.popupType;
    let customClasses = ifefPopup.customClasses;
    let onclickCancel = e => {
      this.cancelAction(e);
    };
      ...
      case 'prompt':
        ...
        template = (
          <span>
            {template}
            <input
              type={ifefPopup.inputType}
              placeholder={ifefPopup.inputPlaceholder}
        ...
        if (button.text) {
          ...
          return (
            <button
              ...
              key={idx}
              onClick={e => self.buttonClicked(e, callback)}
            >
              {button.text}
            </button>
          );
    ...
    let backdropClasses = classnames({ backdrop: willMount, 'visible active': this.state.isUp });
    let classes = classnames({
      'asf-popup-container': willMount,
      'asf-popup-showing': this.state.isUp,
      active: this.state.isUp && !this.state.closing,
      'asf-popup-hidden': this.state.closing,
    });
    let popupClasses = classnames('asf-popup', customClasses);

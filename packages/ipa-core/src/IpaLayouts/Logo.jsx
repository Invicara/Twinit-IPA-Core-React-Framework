...
const Logo = ({ appName, children, homepage, contextProps }) => {
  ...
  useEffect(() => {
    const getImgSrc = async settings => {
      ...
      else if (imgSettings && imgSettings.url) setImgSrc(imgSettings.url);
    ...
    let imgSettings =
      contextProps.userConfig.settings && contextProps.userConfig.settings.appImage
        ? contextProps.userConfig.settings.appImage
        : null;

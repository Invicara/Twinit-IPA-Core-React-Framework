...
const ProjectPickerModal = props => {
  ...
  const getUserGroupOptions = projectid => {
    return appUserGroups[projectid]
      ? appUserGroups[projectid].map(ug => {
          return { value: ug._id, label: ug._name }
        })
      : []
  }
      ...
      if (project._id === selectedProjectId) {
        ...
        projectLoadHandlerCallback &&
          projectLoadHandlerCallback({
            ...
            userGroup: projectUserGroups?.find(
    ...
    const loadModal = async () => {
      ...
      if (projects && projects.length > 0) {
        ...
        for (let i = 0; i < projects.length; i++) {
          let userGroups = await IafProj.getUserGroupsForCurrentUser(
            projects[i]
          )
          ...
          if (userGroups)
            userGroups = userGroups.filter(
              ug => !!ug._userAttributes.userConfigs
            )
          else {
          ...
          let userConfigs = await IafProj.getUserConfigs(projects[i], {
            _userType: configUserType
          })
        ...
        if (
          !appContextProps.selectedItems.selectedProject ||
          !myUserGroups[appContextProps.selectedItems.selectedProject._id]
        ) {
          ...
          usergroupid = projectid
            ? myUserGroups[myProjects[0]._id][0]._id
            : null
        } else {
          ...
          else usergroupid = myUserGroups[myProjects[0]._id][0]._id
        }

        const selectUserGroupOptions = myUserGroups[projectid]
          ? myUserGroups[projectid].map(ug => ({
              value: ug._id,
              label: ug._name
            }))
          : []
        ...
        setUserGroupOptions(selectUserGroupOptions)
        setUserGroupValue(
          _.find(selectUserGroupOptions, { value: usergroupid })
        )
    ...
    <div>
      ...
      <div className='button-container'>
        ...
        <button
          ...
          className={referenceAppConfig?.refApp ? 'load' : 'default-load'}
        >
          Load Project
        </button>
  ...
  if (selectedProjectId)
    defaultProjectOption = _.find(selectProjectOptions, {
  ...
  const buildInviteTable = (invitesForTable, expired) => (
    <SimpleTable
      ...
      rows={invitesForTable.map(inv => {
        ...
        let statusOrButtons = status ? (
          <span className={'invite-state-' + status.toLowerCase()}>
            {status}
          </span>
        ) : (
          <div>
        ...
        <div className='project-picker-modal'>
          ...
          <div>
            ...
            {expiredInvites && expiredInvites.length > 0 && (
              <div>{buildInviteTable(expiredInvites, true)}</div>
            )}
          </div>

...
const PROJECT_PICKER_SELECT_STYLE_OVERRIDES = {
  ...
  container: 'select-container select-input-container project-picker-modal-single-select__inner',
  trigger: 'select-trigger',
  popup: 'select-popup',
};

/** Tooltip for “Choose a user group” — Figma Sign-in / Project access (node 2822:1441). */
const USER_GROUP_FIELD_TOOLTIP = 'Your user group controls what you can see and edit in the app';
const InviteTable = ({ invites, expired, inviteStatus, acceptInvite, rejectInvite }) => (
  <SimpleTable
    ...
    rows={invites.map((inv, i) => {
        ...
        <div className="invite-actions">
          {!expired && (
            <Tooltip title="Accept Invite" placement="top" arrow>
              <span
                className="invite-action invite-action--accept"
      ...
      return [<span key={i}>&bull;</span>, inv._params.name, inv._usergroup._name, statusOrButtons];
    })}
  />
);
...
const ProjectPickerModal = props => {
  ...
  const setCssVariables = async () => {
    ...
    if (referenceAppConfig?.refApp) {
      ...
      const fancytreeOneColor = result.themes.login === 'mirrana' ? '#E04F29' : '#4bade8';
      const fancytreeOneChannelColor = result.themes.login === 'mirrana' ? '#e98469' : '#dbecee';
  ...
  const loadProject = async project => {
    let userConfigs = [];
    let selectedUserGroup = _.filter(userGroups, u => u._id === selectedUserGroupId)[0];
      ...
      if (project._id === selectedProjectId) {
        ...
        projectLoadHandlerCallback &&
          projectLoadHandlerCallback({
            ...
            userGroup: userGroups?.find(UG => UG._id === selectedUserGroupIdLocal),
          });
  ...
  const filterUserGroups = (projectUserGroups, projectUserConfigsMap) => {
    ...
    return projectUserGroups.filter(userGroup => {
      ...
      let userConfigsWithMatchingUserType = userGroup._userAttributes?.userConfigs.filter(
        userConfig => {
  ...
  const fetchUserGroups = async (projects, selectedProjectIdLocal) => {
    ...
    const selectedProject = projects.find(project => project._id === selectedProjectIdLocal);
  ...
  return (
    <Dialog
      ...
      styleOverrides={{
        ...
        title: !hasExistingProject ? 'dialog-title dialog-title__no-close-button' : 'dialog-title',
          ...
          <div className="project-picker-modal__scroll">
            {!loading && (!projects || projects.length === 0) && (
              <div>
                ...
                {!hasPendingInvites && <span> contact your project admin for an invite</span>}
                {referenceAppConfig?.refApp && user?.has_access && (
                  <div>
                    <Button onClick={() => referenceAppCreateProject(projects)}>
                      Create Project
                    </Button>
                ...
                {hasPendingInvites && <span> accept an invite</span>}
              </div>
            )}
            ...
            {referenceAppConfig?.refApp && !loadingProjects && !user?.has_access ? (
              <>
                <br />
                <div>
                  <span className="text-danger">
                    You don't have permission to create a project in the Reference App. Please
                    contact the Admin.
                  </span>
                </div>
                <Button variant="secondary" onClick={userLogout}>
            ...
            {!loadingProjects && projects && projects.length > 0 && (
              <div>
                ...
                {userGroups && userGroups.length > 0 && (
                  <div>
                    <div
                      className={`usergroup-select-container ${loadingUserGroups ? 'hidden' : ''}`}
                    >
                    ...
                    {loadingUserGroups && <SimpleTextThrobber throbberText="Loading user groups" />}
                  </div>
                )}
                ...
                {!loadingUserGroups && userGroups && userGroups.length === 0 && (
                  <div className="project-picker-modal-no-usergroups">
                    <p>
                      No user groups available for this project. Please choose another project or
      ...
      footer={
        <div className="button-container">
          <Button
            ...
            disabled={
              !selectedProjectId || !selectedUserGroupId || loadingUserGroups || loadingProjects
            }

...
export const entitiesSliceFactory = (identifier = '') => {
  ...
  const slice = createSlice({
    ...
    reducers: {
      ...
      deleteEntity: (state, { payload: entity }) => {
        state.allCurrent.splice(
          state.allCurrent.findIndex(e => e._id === entity._id),
          1
        );
      },
      ...
      clearEntities: state => {
        ...
        state.appliedGroups = [];
        state.selectedIds = [];
        ((state.isolatedIds = []), (state.filteredBySearchEntityIds = []));
      },
      clearForNewEntityType: (state, { payload: type }) => {
        ...
        state.selectedIds = [];
        state.isolatedIds = [];
        ((state.currentEntityType = type), (state.filteredBySearchEntityIds = []));
      },
  ...
  const getIsolatedEntitiesIds = createSelector(
    getEntitiesSlice,
    entitiesSlice => entitiesSlice.isolatedIds || []
  );

  const getSelectedEntitiesIds = createSelector(
    getEntitiesSlice,
    entitiesSlice => entitiesSlice.selectedIds || []
  );

  const getFilteredBySearchEntityIds = createSelector(
    getEntitiesSlice,
    entitiesSlice => entitiesSlice.filteredBySearchEntityIds || []
  );

  const fromIDs = (entities, ids) => entities.filter(e => _.includes(ids, e._id));

  //Public Selectors
  const getAllCurrentEntities = createSelector(
    getEntitiesSlice,
    entitiesSlice => entitiesSlice.allCurrent
  );

  const getAppliedFilters = createSelector(
    getEntitiesSlice,
    entitiesSlice => entitiesSlice.appliedFilters
  );

  const getAppliedGroups = createSelector(
    getEntitiesSlice,
    entitiesSlice => entitiesSlice.appliedGroups
  );

  const getAppliedRelatedGroups = createSelector(
    getEntitiesSlice,
    entitiesSlice => entitiesSlice.appliedRelateGroups
  );

  const getFilteredEntities = createSelector(
    [getAllCurrentEntities, getAppliedFilters],
    (currentEntities, appliedFilters) =>
      _.isEmpty(getAppliedFilters)
        ? currentEntities
        : getFilteredEntitiesBy(currentEntities, appliedFilters)
  );

  const getIsolatedEntities = createSelector(
    [getFilteredEntities, getIsolatedEntitiesIds],
    fromIDs
  );

  const getSelectedEntities = createSelector(
    [getFilteredEntities, getSelectedEntitiesIds],
    fromIDs
  );

  const getFetchingCurrent = createSelector(
    getEntitiesSlice,
    entitiesSlice => !!entitiesSlice.fetchingCurrent
  );

  const isViewerSyncOn = createSelector(
    getEntitiesSlice,
    entitiesSlice => entitiesSlice.viewerSyncOn
  );

  const isSelectingEntities = createSelector(
    getEntitiesSlice,
    entitiesSlice => entitiesSlice.selectingEntities
  );

  const getCurrentEntityType = createSelector(
    getEntitiesSlice,
    entitiesSlice => entitiesSlice.currentEntityType
  );
  ...
  const selectEntitiesFromModels = modelEntities => async (dispatch, getState) => {
    try {
      ...
      const entitiesToSelect = await Promise.all(
        modelEntities.map(modelEntity => {
          const foundEntity = getAllCurrentEntities(getState()).find(
            e => modelEntity.id === e.modelViewerIds[0]
          );
          return !_.isEmpty(foundEntity)
            ? new Promise(resolve => resolve(foundEntity))
      ...
      if (
        !setIncludesBy(
          getAllCurrentEntities(getState()),
          filteredToSelect,
          e => e?.modelData?.id || e?._id
        )
      ) {
  ...
  const changeEntity = (changeType, entity) => async (dispatch, getState) => {
    const defaultChangeHandler = () =>
      console.warn('Tried to edt entity with an unknown change type'); //TODO Revisit if not doing anything is ok or we should be throwing an error.
    const entityChanges = {
  ...
  const fetchEntities =
    (script, selector, value, runScriptOptions, initialPageLoad) => async (dispatch, getState) => {

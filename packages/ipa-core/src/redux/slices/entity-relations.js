...
export const sameRelation = (relation, other) =>
  relation.childId === other.childId && relation.parentId === other.parentId;

const getRelation = (related, state, entityId) =>
  related.isChild
...
const entityRelationsSlice = createSlice({
  ...
  reducers: {
    setParentEntities: (state, { payload: entities }) => {
      ...
      state.entitiesById = entities.reduce(
        (byIds, entity) => ({ ...byIds, [entity._id]: entity }),
        {}
      );
      state.entityIds = entities.map(({ _id }) => _id);
    },
    setRelatedEntities: (state, { payload: entities }) => {
      state.relatedById = entities.reduce(
        (byIds, entity) => ({ ...byIds, [entity._id]: entity }),
        {}
      );
    },
      ...
      entityIds.forEach(entityId => {
        related.forEach(r => {
          const newRelation = relatedType.isChild
            ? { parentId: entityId, childId: r._id, new: true }
            : { parentId: r._id, childId: entityId, new: true };
          if (!state.relations.some(r => sameRelation(r, newRelation)))
...
export const getEntityRelations = createSelector(
  getEntityRelationsSlice,
  relationSlice => relationSlice.relations
);

export const getAllEntities = createSelector(getEntityRelationsSlice, relationSlice => ({
  ...relationSlice.entitiesById,
  ...relationSlice.relatedById,
}));

//Public selectors
export const getParentEntities = createSelector(getEntityRelationsSlice, relationSlice =>
  relationSlice.entityIds.map(id => ({
...
export const getFetchingRelatedEntities = createSelector(
  getEntityRelationsSlice,
  relationSlice => relationSlice.fetching
);

export const getEntitiesChanged = createSelector(
  getEntityRelationsSlice,
...
function getRelated(entity, relatedTypes) {
  ...
  const children = _.pick(
    entity,
    relatedTypes.map(ct => ct.singular)
  );
  return _.values(
    _.mapValues(children, (childrenByType, type) =>
      childrenByType._list.map(child => ({
        ...child,
        entityType: type,
        entityName: _.get(child, namesByType[type]),
      }))
    )
  ).flat();
}
...
//Thunks
export const retrieveRelated =
  (originalParentEntities, getScript, relatedTypes) => async (dispatch, getState) => {
    ...
    const related = _.defaultTo(rawEntitiesWithRelated, []).flatMap(entity =>
      getRelated(entity, relatedTypes)
    );
    const relations = _.defaultTo(rawEntitiesWithRelated, []).flatMap(entity =>
      getRelated(entity, relatedTypes).map(relatedEntity =>
        relatedTypes.find(rt => rt.singular === relatedEntity.entityType).isChild
          ? { parentId: entity._id, childId: relatedEntity._id }
          : { parentId: relatedEntity._id, childId: entity._id }
      )
    );
...
const getFilteredParentsWithChildren = (getState, predicate) =>
  _.values(
    _.mapValues(
      _.groupBy(getEntityRelations(getState()).filter(predicate), r => r.parentId),
      (relations, parentId) => ({
        ...getAllEntities(getState())[parentId],
        childrenMap: _.groupBy(
          relations.map(r => getAllEntities(getState())[r.childId]),
          c => c.entityType
        ),
      })
    )
...
export const applyRelationChanges = (script, relatedTypes) => async (dispatch, getState) => {
  const parentEntitiesWithNewChildren = getFilteredParentsWithChildren(
    getState,
    relation => relation.new
  );
  const parentEntitiesWithDeletedChildren = getFilteredParentsWithChildren(
    getState,
    relation => relation.removed
  );
  const parentTypes = _.uniq(
    [...parentEntitiesWithDeletedChildren, ...parentEntitiesWithNewChildren].map(
      pe => pe.entityType
    )
  );
  await Promise.all(
    [...relatedTypes, getCurrentEntityType(getState()).singular].map(childrenType =>
      parentTypes.map(parentType => {
        let params = {
          ...
          parentEntitiesWithNewChildren: parentEntitiesWithNewChildren.filter(
            p => p.entityType === parentType && !_.isEmpty(p.childrenMap[childrenType])
          ),
          parentEntitiesWithDeletedChildren: parentEntitiesWithDeletedChildren.filter(
            p => p.entityType === parentType && !_.isEmpty(p.childrenMap[childrenType])
          ),

...
export const getEntitySelectConfig = createSelector(getUserConfigSlice, config =>
  Object.entries(config.handlers)
    .filter(
      e =>
        e[1].config && e[1].config.entityData && Object.entries(e[1].config.entityData).length === 1
    )
    .map(e => ({

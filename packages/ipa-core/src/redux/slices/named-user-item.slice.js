...
export const fetchAllNamedUserItems = createAsyncThunk(
  ...
  async (args, thunkAPI) => {
    ...
    if (!scriptName) {
      const criteria = {
        query: { _itemClass: { $in: ['NamedUserCollection', 'NamedFileCollection'] } },
      };
      const ctx = { ...args.ctx };
...
export const fileImport = createAsyncThunk('importData/importStatus', async (args, thunkAPI) => {
...
export const selectAllNamedUserItem = createSelector(getNamedUserItemSlice, selectAll);
export const selectNamedUserItemEntities = createSelector(getNamedUserItemSlice, selectEntities);

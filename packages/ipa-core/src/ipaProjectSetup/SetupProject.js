...
export default class SetUpProject extends React.Component {
    ...
    try {
      if (projects !== undefined) {
        for (let i = 0; i < projects.length; i++) {
          if (projects[i]._metadata._createdById === user._id) {
  ...
  async createScripts(projectList, ctx) {
    ...
    for (let i = 0; i < scriptNames.length; i++) {
      ...
      const scriptContent =
        await scriptFiles.files[`setupScripts/${scriptName}.js`].async('string');
    ...
    let scriptItems = [];
    scripts.forEach(c => {
      let item = scriptsDescriptors.find(obj => obj._shortName === c.scriptName);
      if (item) {
  ...
  handleProjectSetUp = async (e, restartApp) => {
    ...
    async function runOrchestrator() {
      ...
      let runSetupOrch = projOrchList._list.find(o => o._userType === 'setup_runner');
      ...
      const map_orch_timer = setInterval(async () => {
        let status = await IafScriptEngine.getDatasourceRunStatus(orchReqForRes, ctx);
        ...
        switch (status._status) {
          ...
          case 'RUNNING': {
            ...
            console.log(`Project setup running since ${(diffInSeconds / 60).toFixed(2)} minutes.`);
            ...
            <div className="project-picker-modal">
              {!this.state.open && (
                <form
                  style={{ width: '100%' }}
                  onSubmit={
                    !this.props.allowMultipleProjects
                      ? this.handleClickOpen
                      : this.handleProjectSetUp
                  }
                >
                  ...
                  >
                    {this.state.click ? (
                      ...
                      <>
                        <button onClick={() => this.props.restartApp()} className="load">
                          Cancel
                        </button>
              ...
              {this.state.open && !this.state.isDeleting && !this.state.isDoneButtonClicked && (
                <div style={{ float: 'left' }}>
                  By Clicking on Agree button, the previously created projects by you would be
                  deleted and this new project would be created.
                  <div>

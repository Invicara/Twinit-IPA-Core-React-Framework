...
export const getFilteredEntitiesBy = (entities, filters) =>
  applyFilters(entities, filters, (a, p) => {
    return p == 'Entity Name' ? a['Entity Name'] : a.properties[p];
  });

export const getEntityFromModel = async (script, modelEntity) => {
  //TODO fix script so that it returns model id if possible

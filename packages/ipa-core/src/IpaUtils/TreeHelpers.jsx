...
export const propagateNodeStatusDown = property => (nodeIndex, nodeId, newNodeStatus) => {
  ...
  if (selectedNode) {
    ...
    const off =
      _.get(
        selectedNode,
        `treeActions.${property}.${TreeNodeActionName.PROPAGATE_DOWN}`,
        TreeNodeStatus.ON
      ) === TreeNodeStatus.OFF;
    if (off) {
      return;
    }
    selectedNode.children.forEach(childName =>
      propagateNodeStatusDown(property)(nodeIndex, childName, newNodeStatus)
    );
  }
};
...
export const propagateNodeStatusUp = property => (nodeIndex, nodeId) => {
  ...
  const off =
    _.get(
      selectedNode,
      `treeActions.${property}.${TreeNodeActionName.PROPAGATE_UP}`,
      TreeNodeStatus.ON
    ) === TreeNodeStatus.OFF;
  if (off) {
    return;
  ...
  selectedNode.parents
    .slice()
    .reverse()
    .forEach(parentName => recalculateNodeStatus(property)(nodeIndex, parentName));
};

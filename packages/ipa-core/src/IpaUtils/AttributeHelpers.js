const specialAttributes = ['<<TEXT>>', '<<DYNAMIC>>'];
...
function cleanAttributes(attributes) {
  ...
  keys.forEach(key => {
    if (Array.isArray(attributes[key] && attributes[key].length === 0)) delete newAttributes[key];
    else if (!attributes[key]) delete newAttributes[key];
  });

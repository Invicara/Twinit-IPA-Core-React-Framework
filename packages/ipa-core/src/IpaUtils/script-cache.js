...
const getCachingArguments = args => {
  let expirationPartition = _.partition(args, function (e) {
    return typeof e !== 'undefined' && e.hasOwnProperty(SCRIPT_EXPIRATION_ARGS_KEY);
  });
  const scriptExpiration = expirationPartition[0];
  let scriptArgs = _.partition(expirationPartition[1], function (e) {
    return typeof e !== 'undefined' && e.hasOwnProperty(IGNORE_CACHED_RESULT_ARGS_KEY);
  });
  return {
    scriptArgs: scriptArgs[1],
    scriptExpiration:
      sessionStorage.getItem(SCRIPT_EXPIRATION_MINUTES_KEY) ||
      (scriptExpiration.length && scriptExpiration[0].hasOwnProperty('scriptExpiration')
        ? scriptExpiration[0].scriptExpiration
        : DEFAULT_EXPIRATION_MINUTES),
    ignoreCachedResult:
      scriptArgs[0] && scriptArgs[0].length ? scriptArgs[0][0].ignoreCachedScriptResult : false,
  };
};

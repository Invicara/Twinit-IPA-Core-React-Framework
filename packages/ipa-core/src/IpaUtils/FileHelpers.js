    ...
    fileItems.map(async fileItem => {
      ...
      if (fileItemName && urlObj._name) {
        ...
        if (
          fileItemExt &&
          urlObjExt &&
          fileItemExt.toLowerCase() === urlObjExt.toLowerCase() &&
          fileItemExt !== urlObjExt
        ) {
...
export async function downloadDocumentsVersions(fileItems) {
  ...
  for (let i = 0; i < fileItems.length; i++) {
    urlPromises.push(
      IafFileSvc.getFileVersionUrl(fileItems[i]._fileId, fileItems[i]._fileVersionId).then(
        urlObj => {
  ...
  Promise.all(urlPromises).then(async () => {
    if (fileUrls.length === 1) window.location.href = fileUrls[0]._url;
    else if (fileUrls.length > 1) {
      let urlFetchPromises = [];
      let allFileBuffs = {};
      for (let i = 0; i < fileUrls.length; i++) {
        urlFetchPromises.push(
          await fetch(fileUrls[i]._url)
            .then(resp => {
...
export const getFileUrlForFilename = async filename => {
  ...
  return IafFile.getFileItems(project.rootContainer, { name: filename }).then(fileItems => {
    ...
    return IafFileSvc.getFileVersionUrl(fileItem._fileId, fileVersion._fileVersionId).then(
      result => {
        return result._url;
      }
    );
...
export const getFileUrl = async (fileId, versionId) => {
  let result = await IafFileSvc.getFileVersionUrl(
    fileId,
    versionId ? versionId : await IafFileSvc.getFileVersions(fileId)._list[0]._id
  );
  return result;
};
